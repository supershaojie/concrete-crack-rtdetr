...
"""GSDR-AIFI v2: corrected optimizer naming, construction RNG, and FP32 attention."""
...
__all__ = ("GSDRAIFIV2", "SparseDeformableRelationV2")
...
class SparseDeformableRelationV2(nn.Module):
    ...
    def __init__(
        ...
        # The trainer routes any full parameter name containing 'bias' into its bias group.
        # Keep this MODULE path neutral so its Linear weights receive ordinary weight treatment.
        self.position_mlp = nn.ModuleList(_ContinuousRelativeBias(32) for _ in range(offset_groups))
    ...
    def _relative_displacement_bias(
        ...
        group_biases = []
        for group, mlp in enumerate(self.position_mlp):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # float() operands alone do not prevent CUDA autocast from selecting FP16 matmul.
        # CPU FP32 uses no autocast context (compatible with PyTorch 2.1).
        context = torch.autocast(device_type="cuda", enabled=False) if x.is_cuda else nullcontext()
        with context:
            ...
            related = torch.matmul(attention, value.float())
        related = related.to(dtype=x.dtype)
...
class GSDRAIFIV2(AIFI):
    ...
    def __init__(
        ...
        self.c1 = c1
        # Only the new branch is forked; the original AIFI keeps the C2 RNG sequence.
        with torch.random.fork_rng(devices=[]):
            self.sparse_relation = SparseDeformableRelationV2(
                c1=c1,
                aux_dim=aux_dim,
                aux_heads=aux_heads,
                offset_groups=offset_groups,
