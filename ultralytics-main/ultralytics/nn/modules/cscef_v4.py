...
"""Structure-consistent, conflict-suppressed cross-scale edge fusion module."""
...
class CSCEFv4(nn.Module):
    """Enhance a lateral feature with structure-confident, conflict-suppressed Scharr edges.

    Both inputs must have the same channel count so one bias-free projection can embed them. The semantic input is
    resized to the lateral spatial resolution when necessary. The output always matches the lateral input's shape,
    dtype, and device.
    ...
    def __init__(
        ...
        if not isinstance(c_lateral, int) or not isinstance(c_semantic, int) or c_lateral <= 0 or c_semantic <= 0:
            raise ValueError("CSCEFv4 input channel counts must be positive integers.")
        if c_lateral != c_semantic:
            raise ValueError(
                "CSCEFv4 requires equal lateral and semantic channel counts for its shared projection, "
        ...
        if not isinstance(hidden, int) or hidden <= 0:
            raise ValueError("CSCEFv4 hidden_channels must be a positive integer.")
        if not isinstance(num_groups, int) or num_groups <= 0 or hidden % num_groups:
            raise ValueError("CSCEFv4 num_groups must be a positive divisor of hidden_channels.")
        if not math.isfinite(eps) or eps <= 0:
            raise ValueError("CSCEFv4 eps must be finite and positive.")
        if not all(math.isfinite(value) for value in (alpha_init, alpha_max)):
            raise ValueError("CSCEFv4 alpha initialization and bound must be finite.")
        if not 0 < alpha_init < alpha_max:
            raise ValueError("CSCEFv4 requires 0 < alpha_init < alpha_max.")
        ...
        # Match CSCEF-v3 exactly: each fixed Scharr kernel is divided by its L1 norm (32).
    ...
    def _project_features(
        ...
        if x_lateral.ndim != 4 or x_semantic.ndim != 4:
            raise ValueError("CSCEFv4 inputs must be 4D BCHW tensors.")
        if x_lateral.shape[1] != self.in_channels or x_semantic.shape[1] != self.in_channels:
            raise ValueError(
                f"CSCEFv4 expected both inputs to have {self.in_channels} channels, but received "
                f"{x_lateral.shape[1]} and {x_semantic.shape[1]}."
            )
        if min(*x_lateral.shape[-2:], *x_semantic.shape[-2:]) <= 0:
            raise ValueError("CSCEFv4 inputs must have non-empty spatial dimensions.")
        if x_lateral.device != x_semantic.device:
            raise ValueError("CSCEFv4 inputs must be on the same device.")
    ...
    def _compute_semantic_bandpass(
        self, lateral_embedding: torch.Tensor, semantic_embedding: torch.Tensor
    ) -> torch.Tensor:
        """Return the FP32 channel-centered cosine bandpass in ``B x 1 x H x W`` form."""
        if lateral_embedding.shape != semantic_embedding.shape:
            raise ValueError("CSCEFv4 projected features must have identical shapes before semantic gating.")
        ...
        with torch.autocast(device_type=lateral_embedding.device.type, enabled=False):
            ...
            return (1.0 - cosine.square()).clamp(0.0, 1.0)

    def _compute_scharr_components(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Return fixed per-channel Scharr-X/Y components in FP32 after one shared padding operation."""
        if x.ndim != 4:
            raise ValueError("CSCEFv4 Scharr input must be a 4D BCHW tensor.")
        if min(x.shape[-2:]) <= 0:
            raise ValueError("CSCEFv4 Scharr input must have non-empty spatial dimensions.")
        ...
        with torch.autocast(device_type=x.device.type, enabled=False):
            ...
            gradient_x, gradient_y = paired_gradients.reshape(
                x_fp32.shape[0], channels, 2, *x_fp32.shape[-2:]
            ).unbind(dim=2)
    ...
    def _compute_structure_terms(
        ...
        """Return FP32 local structure coherence and relative energy reliability."""
        if gradient_x.ndim != 4 or gradient_x.shape != gradient_y.shape:
            raise ValueError("CSCEFv4 Scharr components must be same-shaped 4D BCHW tensors.")
        if min(gradient_x.shape[-2:]) <= 0:
            raise ValueError("CSCEFv4 Scharr components must have non-empty spatial dimensions.")
        if gradient_x.device != gradient_y.device:
            raise ValueError("CSCEFv4 Scharr components must be on the same device.")
        ...
        with torch.autocast(device_type=gradient_x.device.type, enabled=False):
            ...
            jxy = F.avg_pool2d(
                (gradient_x_fp32 * gradient_y_fp32).mean(dim=1, keepdim=True), 3, stride=1, padding=1
            )
            energy = jxx + jyy
    ...
    def _compute_structure_confidence(self, gradient_x: torch.Tensor, gradient_y: torch.Tensor) -> torch.Tensor:
        """Return a detached FP32 local structure confidence in ``B x 1 x H x W`` form."""
        coherence, reliability = self._compute_structure_terms(gradient_x, gradient_y)
        confidence = torch.sqrt((coherence * reliability).clamp(0.0, 1.0))
    ...
    def _match_residual_rms(self, x_lateral: torch.Tensor, residual_raw: torch.Tensor) -> torch.Tensor:
        ...
        if x_lateral.shape != residual_raw.shape:
            raise ValueError("CSCEFv4 lateral and raw residual shapes must match for RMS normalization.")
    ...
    def forward(self, inputs: list[torch.Tensor] | tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        if not isinstance(inputs, (list, tuple)) or len(inputs) != 2:
            raise ValueError("CSCEFv4 expects [x_lateral, x_semantic].")
        x_lateral, x_semantic = inputs
        lateral_embedding, semantic_embedding = self._project_features(x_lateral, x_semantic)
        semantic_bandpass = self._compute_semantic_bandpass(lateral_embedding, semantic_embedding)

        gradient_x, gradient_y = self._compute_scharr_components(lateral_embedding)
        edge_magnitude = torch.sqrt(gradient_x.square() + gradient_y.square() + self.eps)
        structure_confidence = self._compute_structure_confidence(gradient_x, gradient_y)
        gate = (semantic_bandpass * structure_confidence).clamp(0.0, 1.0)
        ...
        return x_lateral + delta.to(device=x_lateral.device, dtype=x_lateral.dtype)
