...
from sci_adapter import ROOT,VARIANTS,YAML,write_json,SOURCE_SHA256
import train_sci_adapter as train
import sci_adapter_results as results


class LifecycleTests(unittest.TestCase):
    def test_all_109_recipe_fields_and_types(self):
        p=ROOT/'docs/sci_adapter/c2_args.yaml'
        ...
        with tempfile.TemporaryDirectory(dir=ROOT/'outputs') as temp:
            ...
            with self.assertRaises(RuntimeError):train.recipe(bad,'scca_sci_cscef_v51_cbr',bad.with_suffix('.pt'))
    ...
    def test_variants_do_not_share_mutable_paths(self):
        ...
        for key in ('run','launch','init','session','lock'):
            self.assertEqual(len(set(p[key] for p in all_paths)),3)
    ...
    def test_test_rejects_unvalidated_or_different_checkpoint(self):
        with tempfile.TemporaryDirectory(dir=ROOT/'outputs') as temp:
            ...
            with patch.object(results,'runtime',return_value={'commit':'abc'}):
                with self.assertRaises(RuntimeError):results.evaluate('scca_sci_cscef_v51_cbr',w,data,'test',r/'test')
                prior=r/'val.json';write_json(prior,dict(status='completed',split='val',checkpoint_sha256='wrong'))
                with self.assertRaises(RuntimeError):results.evaluate('scca_sci_cscef_v51_cbr',w,data,'test',r/'test',val_report=prior)
    ...
    def test_pack_does_not_fill_missing_evaluations(self):
        with tempfile.TemporaryDirectory(dir=ROOT/'outputs') as temp:
            ...
            with patch.object(results,'paths',return_value=p),patch.object(results,'evaluate',side_effect=AssertionError('pack invoked evaluation')):
                with self.assertRaises(RuntimeError):results.package('scca_sci_cscef_v51_cbr',r/'pack.tar.gz')
    ...
    def test_start_dispatches_only_after_preflight_and_protects_repeat(self):
        from types import SimpleNamespace
        from sci_adapter import build
        with tempfile.TemporaryDirectory(dir=ROOT/'outputs') as temp:
            r=Path(temp);data=r/'data.yaml';YAML.save(data,YAML.load(ROOT/'docs/sci_adapter/c2_data.yaml'))
            p=dict(variant='scca_sci_cscef_v51_cbr',name='fixture',run=r/'run',launch=r/'launch',init=r/'init.pt',source=r/'source.pt',
                   c2_args=ROOT/'docs/sci_adapter/c2_args.yaml',session='triad-fixture',lock=r/'lock')
            ...
            with ExitStack() as stack:
                for module,name,value in ((train,'paths',lambda v:p),(train,'runtime',lambda:dict(commit='a'*40,dirty=False)),
                    ...
                    (train,'RTDETR',lambda *a:SimpleNamespace(model=build('scca_sci_cscef_v51_cbr')))):
                ...
                stack.enter_context(patch('audit_sci_adapter.run',return_value=dict(status='passed')))
                stack.enter_context(patch('check_sci_adapter.smoke',side_effect=smoke))
                stack.enter_context(patch('check_sci_adapter.precision',return_value=dict(status='passed')))
                stack.enter_context(patch('check_sci_adapter_gradients.run_checks',return_value=dict(status='passed')))
                train.start_direct('scca_sci_cscef_v51_cbr')
                with self.assertRaises(RuntimeError):train.start_direct('scca_sci_cscef_v51_cbr')
            ...
            self.assertIn('train_sci_adapter.py',script)
    ...
    def test_wrong_worker_token_cannot_overwrite_existing_exit(self):
        with tempfile.TemporaryDirectory(dir=ROOT/'outputs') as temp:
            ...
            with patch.object(train,'paths',return_value=dict(launch=launch)):
                with self.assertRaises(RuntimeError):train.worker('scca_sci_cscef_v51_cbr','wrong')
