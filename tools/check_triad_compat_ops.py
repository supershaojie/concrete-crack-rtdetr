...
import unittest
import torch
from triad_compat import ROOT,BASE_COMMIT,require,write_json
...
case "$*" in
 ...
 'fetch origin codex/rtdetr-triad-compat'|'cat-file -e '*) : ;;
...
'''
        def invoke(**extra):
            return subprocess.run([bash,'-c',command,'fixture',root.as_posix(),(ROOT/'tools/sync_triad_compat.sh').as_posix(),BASE_COMMIT],
        ...
        for name,env,ok in [('new',{},True),('detached_reuse',{},True),('dirty',{'FIXTURE_DIRTY':' M protected.txt'},False),
            ...
            ('outside_history',{'FIXTURE_ANCESTOR':'no'},False),('wrong_origin',{'FIXTURE_ORIGIN':'https://example.org/wrong.git'},False)]:
            proc=invoke(**env);require((proc.returncode==0)==ok,f'Sync {name}: {proc.stdout} {proc.stderr}')
        pin=root/'worktree/outputs/triad_compat_sync.json';pin.write_text('user file')
        ...
        return dict(status='passed',scope='actual Bash script with explicit Git stub; no network/server/git-worktree mutation',
            ...
            pinned_ancestor_accepted=True,outside_history_rejected=True,wrong_origin_rejected=True,pin_preserved=True)


def run(bash,output):
    loader=unittest.TestLoader();suite=unittest.TestSuite()
    for name in ('test_cscef_v6.py','test_scca_aifi_v2.py','test_cbr_v2.py','test_triad_compat.py','test_triad_lifecycle.py'):
    ...
    for name in ('autodl_triad_compat.sh','sync_triad_compat.sh'):
