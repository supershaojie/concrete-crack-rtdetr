"""Bounded PSDB-P3 engineering preflight; never completes an epoch or evaluates val/test."""
...
from init_psdb_p3 import (ROOT, MODEL_DIR, VARIANTS, build, controlled_models, build_training_model,
...
def branch_grads(model):
    return {name:dict(norm=float(p.grad.detach().double().norm()) if p.grad is not None and torch.isfinite(p.grad).all() else None,
            ...
            for name,p in model.named_parameters() if '.psdb.' in name}


UPSTREAM = ('W_d', 'DW3', 'W_q', 'W_g', 'W_phi_q', 'W_phi_k')


@contextmanager
def strict_fp32_fusion():
    """Diagnostic-only TF32 control; never leaks into native capacity or training."""
    previous = dict(matmul=torch.backends.cuda.matmul.allow_tf32, cudnn=torch.backends.cudnn.allow_tf32)
    try:
        torch.backends.cuda.matmul.allow_tf32 = False
        torch.backends.cudnn.allow_tf32 = False
        yield previous
    finally:
        torch.backends.cuda.matmul.allow_tf32 = previous['matmul']
        torch.backends.cudnn.allow_tf32 = previous['cudnn']
...
def core_checks():
    from check_psdb_p3_core import attention_checks, core_checks as detailed_core_checks
    rows=[]
    ...
    return dict(status='PASSED',phase_cases=rows,odd_core_identity=True,invalid_shape_rejected=True,
                attention=attention_checks(), two_step_gradients=detailed_core_checks(),
...
def wiring_check(target):
    ...
    try:
        ...
        handles.append(model.model[19].psdb.register_forward_pre_hook(inputs('psdb_inputs')))
        ...
        pairs=[('P2_to_wrapper',seen['4'],seen['19_inputs'][0][1]),
               ...
               ('P2_to_core',seen['4'],seen['psdb_inputs'][0]),
        ...
        require(torch.equal(original,seen['psdb_inputs'][1]),'PSDB semantic input must be the original RepC3 output')
...
def loss_startup(target,device,folder,amp=False):
    ...
    for attempt in range(24):
        ...
        before=model.model[19].psdb.W_o.weight.detach().clone();scale_before=scaler.get_scale()
        if finite:
            if effective==0:
                require(grads['model.19.psdb.W_o.weight']['norm']>0,'Initial W_o gradient is zero')
                require(all(v['norm']==0 for k,v in grads.items() if k!='model.19.psdb.W_o.weight'),'Upstream gradient must initially be zero')
                initial=grads
            else:
                for name in UPSTREAM:
                    if grads[f'model.19.psdb.{name}.weight']['norm']>0:upstream_seen.add(name)
            torch.nn.utils.clip_grad_norm_(model.parameters(),10.)
        scaler.step(opt);scaler.update()
        changed=not torch.equal(before,model.model[19].psdb.W_o.weight)
        ...
        del preds,loss
        if effective>=2 and len(upstream_seen)==len(UPSTREAM):break
    require(effective>=2 and len(upstream_seen)==len(UPSTREAM),'Branch did not start after dynamic-scaled optimizer updates')
    require(torch.count_nonzero(model.model[19].psdb.W_o.weight)>0,'Updated W_o still zero')
...
def nonzero_fusion(model,variant,device,folder):
    # Record default precision first. Only the strict diagnostic below changes flags.
    original = dict(matmul=torch.backends.cuda.matmul.allow_tf32, cudnn=torch.backends.cudnn.allow_tf32)
    default_model = deepcopy(model).to(device).eval()
    with torch.no_grad():
        default_model.model[19].psdb.W_o.weight.add_(torch.linspace(-.02,.02,8192,device=device).reshape_as(default_model.model[19].psdb.W_o.weight))
        default_fused = deepcopy(default_model).fuse(verbose=False)
        x = synthetic_batch((160,192),device)['img'][:1]
        _, left = capture(default_model, x)
        _, right = capture(default_fused, x)
    from c19_lif_v1_diagnostic import selection_report
    keys = ['scale_0','scale_1','scale_2','boxes','scores','enc_boxes','enc_scores']
    require(all(torch.isfinite(record[key]).all() for record in (left,right) for key in keys), 'Nonfinite default fusion')
    default = dict(flags=original, selection=selection_report(left,right),
                   max_abs_errors={key:float((left[key].float()-right[key].float()).abs().max()) for key in keys},
                   note='Default native top-k is recorded without treating unordered decoded rows as continuous error.')
    del default_model,default_fused,left,right,x
    gc.collect()
    with strict_fp32_fusion():
        result = _nonzero_fusion_strict(model,variant,device,folder)
    require(original == dict(matmul=torch.backends.cuda.matmul.allow_tf32,cudnn=torch.backends.cudnn.allow_tf32),'TF32 flags not restored')
    result.update(default_precision=default, strict_fp32_tf32_flags=dict(matmul=False,cudnn=False), original_flags_restored=True)
    return result


def _nonzero_fusion_strict(model,variant,device,folder):
    ...
    with torch.no_grad():model.model[19].psdb.W_o.weight.add_(torch.linspace(-.02,.02,8192,device=device).reshape_as(model.model[19].psdb.W_o.weight))
    psdb_state={k:v.clone() for k,v in model.model[19].psdb.state_dict().items()}
    x=synthetic_batch((160,192),device)['img'][:1]
    disabled=deepcopy(model)
    with torch.no_grad():disabled.model[19].psdb.W_o.weight.zero_()
    ...
    require(difference>1e-7,'Nonzero PSDB has no network effect')
    fused=deepcopy(model).fuse(verbose=False)
    state_exact(psdb_state,fused.model[19].psdb.state_dict(),'Fusion changed PSDB/GN')
    require(sum(isinstance(m,torch.nn.GroupNorm) for m in fused.model[19].psdb.modules())==2,'GN lost at fusion')
    ...
    expected=19973829 if variant.startswith('cbr_') else 19906580
    ...
    return dict(status='PASSED',branch_disabled_p3_max_difference=difference,precision=rows,
                ...
                psdb_states_unchanged=True,GN_preserved=True,reload=restored_report,fused_parameters=expected,
...
def server_capacity(initialized,variant,data,folder):
    from train_psdb_p3 import recipe, verify_data
    ...
    class BoundedTrainer(RTDETRTrainer):
        def get_model(self,cfg=None,weights=None,verbose=True):
            model,audit=build_training_model(cfg,weights,self.data,variant);observed['rebuild']=audit
            def decoder_seen(module,inputs,output):
                if module.training:
                    metadata=output[-1]
                    require(metadata and metadata['dn_num_split'][0]>0,'Real training batch did not exercise DN')
                    observed['latest_dn_split']=metadata['dn_num_split']
            model.model[26].register_forward_hook(decoder_seen)
            return model
        def preprocess_batch(self,batch):
            batch=super().preprocess_batch(batch)
            require(torch.isfinite(batch['img']).all(),'Nonfinite augmented training input')
            require(batch['cls'].numel()>0,'Real training batch has no valid GT')
            observed['latest_gt_count']=int(batch['cls'].numel())
            return batch
        ...
        def optimizer_step(self):
            ...
            row=dict(batch=batch_count,loss=float(self.loss.detach()),scale_before=scale,
                     ...
                     gt_count=observed.get('latest_gt_count'),dn_split=observed.get('latest_dn_split'))
            ...
            if updates>before:
                if before==0:
                    require(grads['model.19.psdb.W_o.weight']['norm']>0,'First effective real step has no W_o gradient')
                    require(all(value['norm']==0 for name,value in grads.items() if name!='model.19.psdb.W_o.weight'),
                            'First effective real step has premature upstream gradients')
                else:
                    for name in UPSTREAM:
                        value=grads[f'model.19.psdb.{name}.weight']
    ...
    def batch_end(trainer):
        ...
        if updates>=2 and len(new_seen)==len(UPSTREAM):raise BoundedStop()
    ...
    require(updates>=2 and len(new_seen)==len(UPSTREAM),'Native bounded optimizer updates incomplete')
...
def main():
    ...
    parser.add_argument('--variant',choices=list(VARIANTS),default='cbr_lif_psdb_p3_v1')
    ...
    try:
        ...
        from train_psdb_p3 import code_identity, verify_data
        import ultralytics
        module_path=Path(inspect.getfile(PSDBP3)).resolve()
        ...
        report['imports']=dict(ultralytics=str(Path(ultralytics.__file__).resolve()),psdb=str(module_path))
        ...
        from check_psdb_p3_core import count_checks,warmup_checks
        ...
        for variant in VARIANTS:
            ...
            target.nc=1
            new_states.append({k:v.clone() for k,v in target.state_dict().items() if '.psdb.' in k})
            ...
            expected=20178629 if variant.startswith('cbr_') else 20111636
            ...
            row['complexity']=dict(nc=1,imgsz=640,unfused_parent_parameters=parent_count,unfused_target_parameters=count,
                delta_parameters=count-parent_count,analytical_conv_MACs=193536000,analytical_conv_GFLOPs=.387072,
                scope='PARTIAL: PSDB convolutions including functional q once/shared k four times; GN/L2/dot/softmax/activation/elementwise excluded. Full-network FLOPs not claimed.')
        ...
        state_exact(new_states[0],new_states[1],'Variant PSDB initialization differs')
