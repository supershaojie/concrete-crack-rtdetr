"""Bounded local validation of all seven variants; no dataset loop or formal training."""
...
from init_triad_compat import controlled_models, initialize, rebuild_audit
from check_triad_compat_gradients import run_checks, activate
...
def equivalence(reference,target,variant):
    ...
    roles=topology(target,variant)['roles']; br=baseline_roles(reference.yaml);br['P3_enh']=br['P3_base']
    report={}
    for height,width in ((640,640),(160,192)):
        ...
        for slot,role in enumerate(('P3_enh','P4_base','P5_base')):
            require(bf['decoder_inputs'][slot] is bf[role], f'Decoder does not use {role}')
        if VARIANTS[variant]['cbr']:require(bf['decoder_inputs'][3] is bf['P3_ref'],'CBR reference route wrong')
        if VARIANTS[variant]['cscef']:require(order.index('P3_enh')>order.index('P5_base'),'CSCEF ran before PAN completion')
...
def smoke(target,variant,device='cpu',amp=False):
    ...
    for step,(bs,counts) in enumerate(((2,[2,1]),(1,[3]))):
        ...
        heads={n:v for n,v in added.items() if any(k in n for k in ('output_projection','scca_o','offset_out'))}
        require(all(v>0 for v in heads.values()),'Zero-init output head did not learn')
        if step:require(all(v>0 for v in added.values()),f'Nonzero second-step new parameter gradient missing: {added}')
    ...
    loaded=torch_load(buffer,map_location='cpu')['model'];loaded.model[-1].shapes=None
    ...
    rebuilt=__import__('init_triad_compat').rebuild_nc1(loaded)
    ...
    result=dict(status='passed',device=device,amp=amp,debug_scaler_initial=128 if amp else None,steps=rows,
        optimizer_coverage=coverage,save_load=reload_errors,native_rebuild_preserves_learned=True)
    del model,loaded,rebuilt,optimizer;gc.collect()
...
def routing_interventions(target,variant):
    ...
    for name in ('P3_ref','P4_ref','P3_base','P4_base','P5_base'):
        require(torch.equal(before[name],after[name]),f'CSCEF polluted {name}')
    require(not torch.equal(before['P3_enh'],after['P3_enh']),'CSCEF did not affect decoder P3')
    return dict(status='passed',PAN_unchanged=True,decoder_P3_changed=True,
                P3_enh_max_abs=float((before['P3_enh']-after['P3_enh']).abs().max()))
...
def run(source,output,variants=None):
    ...
    report['gradient_firewall']=run_checks();report['legacy_regression']=legacy_regression()
    write_json(output/'gradient_firewall.json',report['gradient_firewall'])
    for variant in variants or VARIANTS:
        ...
        result['zero_init']=equivalence(reference,target,variant)
