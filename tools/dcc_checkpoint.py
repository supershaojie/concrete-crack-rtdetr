"""DCC-only native checkpoint policy: keep optimizer moments in FP32.
...
def native_serializer_identity():
    ...
    if (identity["native_serializer_sha256"] != NATIVE_SERIALIZER_SOURCE_SHA256 and
        ...
        raise RuntimeError("Native save_model semantics changed; review DCC checkpoint override before saving")
    return identity
...
def require_checkpoint_policy(checkpoint):
    ...
    """
    metadata = checkpoint.get("dcc_checkpoint", {})
    ...
    if not isinstance(ema, torch.nn.Module):
        raise RuntimeError("Native DCC resume requires its original EMA model")
...
class DCCCheckpointTrainer(RTDETRTrainer):
        ...
        for state in optimizer_state["state"].values():
            for name in ("exp_avg", "exp_avg_sq", "max_exp_avg_sq"):
                ...
                if value is not None and (not isinstance(value, torch.Tensor) or value.dtype != torch.float32):
                    raise RuntimeError(
                        "DCC requires active FP32 AdamW moments; old FP16 checkpoint information cannot be recovered by casting"
                    )
