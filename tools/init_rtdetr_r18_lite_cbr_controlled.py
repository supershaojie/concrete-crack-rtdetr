"""Create clean FP32 C19 initialization from the SHA-locked original C2 source. No training."""
...
CBR_CFG = MODEL_DIR / "rtdetr-resnet18-lite-cbr.yaml"
...
DEFAULT_OUTPUT = ROOT / "weights/rtdetr_r18_lite_cbr_imagenet_backbone_init.pt"
...
PROTECTION = ROOT / "docs/cbr_protected.json"
sys.path.insert(0, str(ULTRALYTICS_ROOT))

import ultralytics
from ultralytics import RTDETR
from ultralytics.cfg import DEFAULT_CFG_DICT
from ultralytics.nn.modules import AIFI, RTDETRDecoderCBR
...
def runtime_info():
    ...
    require(imported.is_relative_to(ULTRALYTICS_ROOT.resolve()), "Ultralytics imported outside C19 worktree.")
...
def verify_module(model, require_zero=True):
    ...
    cbr = head.cbr
    require(cbr.rho == .1 and cbr.normal_fraction == .1, "Fixed C19 geometry changed.")
...
def verify_reloads(output, expected):
    ...
    for name, model in (("raw", checkpoint["model"]), ("RTDETR(checkpoint)", RTDETR(str(output)).model),
                        ("RTDETR(YAML).load(checkpoint)", RTDETR(str(CBR_CFG)).load(str(output)).model)):
        verify_module(model)
...
def initialize(source, output):
    ...
    torch.manual_seed(42)
    target = RTDETRDetectionModel(str(CBR_CFG), nc=80, verbose=False).eval()
    ...
    require(extra == {new_prefix(target) + k for k in target.model[-1].cbr.state_dict()}, "Unexpected new states.")
    ...
    target.args = {**DEFAULT_CFG_DICT, "model": str(CBR_CFG), "task": "detect"}
    ...
    checkpoint = {"epoch": -1, "best_fitness": None, "model": deepcopy(target).float(), "ema": None,
                  ...
                  "version": ultralytics.__version__, "license": "AGPL-3.0", "cbr_provenance": provenance}
    output.parent.mkdir(parents=True, exist_ok=True)
    with output.open("xb") as file:
        torch.save(checkpoint, file)
    return {**provenance, "output": str(output), "output_sha256": sha256(output), "common_tensor_audit": rows,
            "new_state_keys": sorted(extra), "new_parameters": sum(p.numel() for p in target.model[-1].cbr.parameters()),
...
def main():
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "outputs/cbr/initialization.json")
