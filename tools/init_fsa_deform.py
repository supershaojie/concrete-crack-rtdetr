...
from ultralytics.nn.modules import AIFI, RTDETRDecoder, RTDETRDecoderFSA, FSADeformAttn
...
VARIANTS = {"fsa_deform": ("rtdetr-resnet18-lite-fsa-deform.yaml", "rtdetr-resnet18-lite.yaml",
                            "fsa_deform_rtdetr_r18_lite_e200_b16_onlineaug")}
PARAMETERS = {"c2": 20082772, "fsa_deform": 20087292}
...
def runtime():
    ...
    source_files = ["ultralytics-main/ultralytics/nn/modules/fsa_deform.py",
                    "ultralytics-main/ultralytics/nn/modules/transformer.py", "ultralytics-main/ultralytics/nn/modules/head.py",
                    "ultralytics-main/ultralytics/nn/modules/utils.py", "ultralytics-main/ultralytics/nn/tasks.py",
                    "ultralytics-main/ultralytics/cfg/models/rt-detr/rtdetr-resnet18-lite-fsa-deform.yaml"]
    return dict(python=sys.version, executable=sys.executable, torch=str(torch.__version__), cuda=torch.version.cuda,
                fsa_module=__import__("inspect").getfile(FSADeformAttn), gpu=torch.cuda.get_device_name() if torch.cuda.is_available() else None,
...
def is_added(key):
    return key.startswith("model.26.decoder.layers.1.cross_attn.support_fc")


def build(variant="fsa_deform", nc=80, baseline=False):
    with torch.random.fork_rng(devices=[]):
        torch.random.default_generator.manual_seed(42)
        return RTDETRDetectionModel(str(MODEL_DIR / VARIANTS[variant][int(baseline)]), nc=nc, verbose=False)


def verify_model(model, variant="fsa_deform", zero=False):
    require(variant == "fsa_deform", "Only FSA single module is supported")
    expected = YAML.load(MODEL_DIR / VARIANTS[variant][1])
    expected["head"][-1][2] = "RTDETRDecoderFSA"
    require(all(model.yaml[k] == expected[k] for k in ("backbone", "head", "scales")), "Not pure C2 + FSA topology")
    head = model.model[-1]
    require(len(model.model) == 27 and type(model.model[9]) is AIFI and type(head) is RTDETRDecoderFSA, "Wrong backbone/AIFI/head")
    require(head.f == [19, 22, 25] and head.num_queries == 300, "Neck/regular queries changed")
    require(head.decoder.num_layers == 3 and head.decoder.eval_idx == 2, "Wrong decoder setup")
    ...
    require([type(l.cross_attn) for l in layers] == [MSDeformAttn, FSADeformAttn, MSDeformAttn], "Replace index=1 cross-attention only")
    require(all(type(l.self_attn) is torch.nn.MultiheadAttention for l in layers), "Self-attention changed")
    fsa_deform = layers[1].cross_attn
    require((fsa_deform.d_model, fsa_deform.n_heads, fsa_deform.n_levels, fsa_deform.n_points) == (256, 8, 3, 4), "C2 sampling dimensions changed")
    require(sum(p.numel() for n, p in model.named_parameters() if is_added(n)) == 4520, "Wrong added count")
    ...
    if zero:
        require(not torch.count_nonzero(fsa_deform.support_fc2.weight) and not torch.count_nonzero(fsa_deform.support_fc2.bias), "Coefficients not zero")
        require(torch.count_nonzero(fsa_deform.support_fc1.weight) and not torch.count_nonzero(fsa_deform.support_fc1.bias), "Scoring projections zero")
...
def controlled_models(source, variant="fsa_deform"):
    source = Path(source)
    ...
    require(set(new) - set(public) == extra and not set(public) - set(new), "Unexpected FSA state gap")
    ...
    from train_fsa_deform import rebuild_audit
    reference1, target1 = rebuild_nc1(reference), rebuild_nc1(target)
    adaptation = rebuild_audit(target, target1, variant)
    require(all(torch.equal(v, target1.state_dict()[k]) for k, v in reference1.state_dict().items()), "C2/FSA nc1 common states differ")
    verify_model(target1, zero=True)
    report = dict(source=str(source.resolve()), source_sha256=SOURCE_SHA256, variant=variant, seed=42,
                  source_states=len(rows), missing_source=[], unexpected_source=[], shape_mismatches=[],
                  new_states=sorted(extra), new_parameters=4520, public_constructor_equal=True,
    ...
    report.update(COMMON=rows, NEW=sorted(extra), MISSING=[], UNEXPECTED=[], SHAPE_MISMATCH=[],
                  ALLOWED_EXCEPTION=adaptation["classification_shape_skips"], c2_commit=BASE_COMMIT,
                  middle_attention={k: torch.equal(v, target1.state_dict()[k]) for k, v in reference1.state_dict().items()
                                    if k.startswith("model.26.decoder.layers.1.cross_attn.")},
                  outer_layers_exact=all(torch.equal(v, target1.state_dict()[k]) for k, v in reference1.state_dict().items()
                                         if k.startswith(("model.26.decoder.layers.0.", "model.26.decoder.layers.2."))))
    return reference1, target1, report


def initialize(source, output, variant="fsa_deform"):
    output = Path(output)
