...
def run(pipeline,checkpoint,output):
    ...
    c2=ROOT/'docs/sci_adapter/c2_args.yaml';shutil.copyfile(c2,launch/'authoritative_c2_args.yaml')
    shutil.copyfile(pipeline/'subset/data.yaml',launch/'data_config.yaml')
    args,diff=recipe(c2,'scca_sci_cscef_v51_cbr',checkpoint)
    for name in ('train_args.yaml','actual_train_args.yaml'):YAML.save(launch/name,args)
    YAML.save(training/'args.yaml',args)
    plan=dict(runtime=info,preflight='passed',args=args,c2_args_sha256=sha256(c2),init_sha256=sha256(checkpoint),variant='scca_sci_cscef_v51_cbr',token=token,fixture=True)
    ...
    for name in ('nc1_loading.json','training_setup.json','parameter_diff.json','amp_resources.json','preflight.json','source_audit.json'):
        write_json(launch/name,dict(status='passed',fixture=True))
    (launch/'sci_adapter_stats.jsonl').write_text('{"epoch":0,"residual_to_Y4_RMS":0.0,"fixture":true}\n')
    ...
    with patch.object(results,'paths',return_value=p),patch.object(results,'evaluate',side_effect=AssertionError('Pack must never run eval')):
        archive=results.package('scca_sci_cscef_v51_cbr',output/'fixture_complete.tar.gz')
        inventory=results.verify_archive(archive)
        names={item['path'] for item in inventory}
        required={'metadata/source/tools/sci_adapter.py',
                  'metadata/source/tools/train_sci_adapter.py',
                  'metadata/source/ultralytics-main/ultralytics/nn/modules/semantic_compat_adapter.py',
                  'metadata/source/docs/sci_adapter/gradient_paths.json'}
        required.update('metadata/source/ultralytics-main/ultralytics/cfg/models/rt-detr/'+v['yaml'] for v in VARIANTS.values())
        require(required <= names, 'SCI source/YAML/gradient evidence missing from package; stage new files before fixture')
        try:results.package('scca_sci_cscef_v51_cbr',archive)
        except RuntimeError:pass
        else:raise AssertionError('Overwrote fixture archive')
        (launch/'evaluation_test/metrics.json').write_text('{}')
        try:results.package('scca_sci_cscef_v51_cbr',output/'invalid.tar.gz')
        except RuntimeError:pass
    ...
    write_json(output/'pack_validation.json',dict(status='passed',scope='explicit synthetic train-lifecycle fixture + real two-image val/test outputs; NOT formal training',
        ...
        no_implicit_evaluation=True,overwrite_rejected=True,broken_test_rejected=True,sci_source_yaml_gradient_evidence=True))
