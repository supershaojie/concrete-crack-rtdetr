"""DRA native C2 evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_dra import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_dra import MAIN, paths
...
POLICY = "native_c2_sorted_original_mask_v1"
EVAL = dict(imgsz=640, batch=16, workers=0, half=False, conf=.001, iou=.7, max_det=300, augment=False, rect=False)


def export_queries(raw, imgsz, conf):
    """Snapshot all queries in native sorted order; retain native pre-sort mask explicitly.

    C2's existing validator applies an unsorted mask after sorting. This task does not
    change that policy. Export flags reproduce its actual metric subset, even at ties.
    """
    full = []
    for pred in raw:
        score, cls = pred[:, 4:].max(-1)
        order = score.argsort(descending=True)
        full.append(dict(bboxes=ops.xywh2xyxy(pred[:, :4] * imgsz)[order],
                         conf=score[order], cls=cls[order].to(pred.dtype), metric_mask=score > conf))
    return full


def image_record(pred, gt, dataset_root, conf):
    """One image, all 300 FP32 predictions and all GT; no decimal rounding/clipping."""
    ...
    return dict(image=rel, original_size_hw=[h, w], box_format="xyxy", coordinate_space="original_image_pixels",
                ...
                prediction_order="native descending score order", metric_subset="used_for_metrics reproduces native original-order mask",
                predictions=[dict(class_id=int(c), score=float(s), bbox=b, used_for_metrics=bool(used))
                             for b, s, c, used in zip(boxes.tolist(), scores, classes, pred["metric_mask"].cpu())],
                ground_truth=[dict(class_id=int(c), bbox=b) for b, c in zip(gt_boxes.tolist(), gt["cls"].detach().cpu())])


def evaluate(weights, data, split, output, device="0", val_report=None):
    ...
    report = dict(status="failed", runtime=info, split=split, settings=settings, checkpoint=str(weights),
                  ...
                  boxes="Original final Decoder layer", selection="Training val selects best; freeze checkpoint/config before independent test",
                  full_server_preflight="NOT_RUN", matching=dict(class_aware=True, iou_thresholds=[round(.5 + .05*i, 2) for i in range(10)],
                    algorithm="Native BaseValidator.match_predictions default greedy IoU matching",
                    precision_recall="Native confidence at maximum mean smoothed F1",
                    source_files=["ultralytics/engine/validator.py", "ultralytics/models/yolo/detect/val.py", "ultralytics/utils/metrics.py"]))
        ...
        with gzip.open(stream_path, "xt", encoding="utf-8") as stream:
            class StreamingValidator(RTDETRValidator):
                ...
                def postprocess(self, preds):
                    ...
                    # Snapshot before native postprocess multiplies bboxes in place.
                    full = export_queries(raw, self.args.imgsz, self.args.conf)
                    ...
                    for selected_image, all_queries in zip(selected, full):
                        ...
                        mask = all_queries["metric_mask"]
                        for key in ("bboxes", "conf", "cls"):
                            require(torch.equal(all_queries[key][mask], selected_image[key]), "Export changed native sorting/mask")
                        counts["historical_mask_affected_images"] += int(not torch.equal(mask, all_queries["conf"] > self.args.conf))
...
def missing_evidence(p):
    required_launch = ("exit_code.json", "process_exit_code.txt", "initialization.json", "nc1_loading.json",
        "training_setup.json", "actual_train_args.yaml", "train_args.yaml", "parameter_diff.json",
        "authoritative_c2_args.yaml", "data_config.yaml", "pip_freeze.txt", "source_snapshot.tar.gz",
        "source_from_base.patch", "source_record.json", "plan.json", "process.json", "console.log", "worker.sh", "launch_state.json")
    files = [p["launch"] / n for n in required_launch]
    files += [p["run"] / n for n in ("weights/best.pt", "weights/last.pt", "args.yaml", "results.csv", "results.png")]
    for split in ("val", "test"):
        files += [p["launch"] / ("evaluation_" + split) / n for n in ("metrics.json", "predictions_gt.jsonl.gz")]
    missing = [str(f) for f in files if not f.is_file()]
    for folder in (p["run"], p["launch"] / "evaluation_val", p["launch"] / "evaluation_test"):
        for pattern in ("*PR_curve.png", "*P_curve.png", "*R_curve.png", "*F1_curve.png",
                        "confusion_matrix.png", "confusion_matrix_normalized.png", "val_batch*pred*"):
            if not any(folder.rglob(pattern)):
                missing.append(str(folder / pattern))
    for pattern in ("labels*", "train_batch*"):
        if not any(p["run"].rglob(pattern)):
            missing.append(str(p["run"] / pattern))
    return missing
...
def validate_complete(p):
    ...
    require(json.loads((launch / "exit_code.json").read_text())["exit_code"] == 0 and
    ...
    required = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml", "train_args.yaml",
                ...
                "source_from_base.patch", "source_record.json", "plan.json", "process.json", "console.log", "worker.sh", "launch_state.json")
    require(all((launch / n).is_file() for n in required), "DRA direct-start metadata incomplete")
    ...
    require(plan["full_server_preflight"] == "NOT_RUN", "Unexpected direct-start preflight record")
    ...
    for split in ("val", "test"):
        ...
        count, predictions, truths, metric_predictions, seen = 0, 0, 0, 0, set()
        with gzip.open(stream, "rt", encoding="utf-8") as f:
            for line in f:
                ...
                metric_predictions += sum(p["used_for_metrics"] for p in row["predictions"])
        ...
        require(metric_predictions == report["metric_predictions"], "Metric subset count mismatch")
...
def package(destination=None):
    """Archive available evidence; declare completeness only after full validation. No implicit inference."""
    p = paths("dra_aifi")
    destination = Path(destination) if destination else MAIN / "downloads/dra" / f"dra_evidence_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    require(all(not Path(str(destination) + suffix).exists() for suffix in ("", ".partial", ".sha256", ".inventory.json", ".verification.json")),
            "Preserve old archive/sidecar/partial evidence")
    launch, run = p["launch"], p["run"]
    missing = missing_evidence(p)
    evaluations = {}
    best_hash = sha256(run / "weights/best.pt") if (run / "weights/best.pt").exists() else None
    if not missing:
        try:
            evaluations = validate_complete(p)
    ...
    tree(run, "training")
    tree(launch, "metadata/launch", ("evaluation_val", "evaluation_test"))
    for split in ("val", "test"):
        tree(launch / ("evaluation_" + split), "evaluation/" + split)
    logs = ROOT / "outputs/dra_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/dra/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/dra/c2_args.yaml") or
                     (name.startswith("tools/") and ("dra" in name or name in ("tools/init_dra.py", "tools/train_dra.py")))):
    ...
    metadata = dict(created=datetime.now(timezone.utc).isoformat(), runtime=runtime(), evaluations=evaluations,
                    training_best_sha256=best_hash, training_last_sha256=sha256(run / "weights/last.pt") if (run / "weights/last.pt").exists() else None,
                    complete=not missing, missing_evidence=missing,
                    unified_init_sha256=SOURCE_SHA256, full_server_preflight="NOT_RUN",
    ...
    write_json(Path(str(destination) + ".verification.json"), dict(archive_sha256=digest, all_member_hashes_verified=True, complete=not missing, missing_evidence=missing, members=len(manifest)))
    print(f"Verified archive (complete={not missing}): {destination}\nBytes: {destination.stat().st_size}\nSHA256: {digest}")
    return destination
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed DRA directories")
        p = paths("dra_aifi")
        require(json.loads((p["launch"] / "exit_code.json").read_text())["exit_code"] == 0 and
                (p["launch"] / "process_exit_code.txt").read_text().strip() == "0", "Formal training not completed successfully")
        plan = json.loads((p["launch"] / "plan.json").read_text())
        require(runtime()["commit"] == plan["runtime"]["commit"], "Evaluation source differs from training source")
