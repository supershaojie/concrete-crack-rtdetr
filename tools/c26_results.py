"""C26 refined-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_c26 import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_c26 import MAIN, paths
from scca_results import postprocess
...
def image_record(pred, gt, dataset_root, conf):
    """One image, all 300 refined FP32 predictions and all GT; no decimal rounding/clipping."""
...
def evaluate(weights, data, split, output, device="0", val_report=None):
    ...
    report = dict(status="failed", runtime=info, split=split, settings=settings, checkpoint=str(weights),
                  ...
                  boxes="CBR-refined final Decoder layer", selection="Training val selects best; freeze checkpoint/config before independent test",
                  full_server_preflight="NOT_RUN", cbr_box_diagnostics="NOT_RUN")
...
def package(destination=None):
    """Stream complete existing outputs to disk. No 20 MiB limit, no implicit inference, no C19 audit gate."""
    p = paths("c26")
    destination = Path(destination) if destination else MAIN / "downloads/c26" / f"c26_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    require(all(not Path(str(destination) + suffix).exists() for suffix in ("", ".partial", ".sha256", ".inventory.json")),
    ...
    required = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml", "train_args.yaml",
                ...
                "source_from_c24.patch", "source_record.json", "plan.json", "process.json", "console.log", "worker.sh", "launch_state.json")
    require(all((launch / n).is_file() for n in required), "C26 direct-start metadata incomplete")
    ...
    logs = ROOT / "outputs/c26_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/c26/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/scca/c2_args.yaml") or
                     (name.startswith("tools/") and ("c26" in name or name in ("tools/init_scca.py", "tools/scca_results.py")))):
    ...
    metadata = dict(created=datetime.now(timezone.utc).isoformat(), runtime=runtime(), evaluations=evaluations,
                    training_best_sha256=best_hash, training_last_sha256=sha256(run / "weights/last.pt"),
                    unified_init_sha256=SOURCE_SHA256, full_server_preflight="NOT_RUN", cbr_box_diagnostics="NOT_RUN",
    ...
    print(f"Verified complete archive: {destination}\nBytes: {destination.stat().st_size}\nSHA256: {digest}")
    return destination
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed C26 directories")
        p = paths("c26")
        ...
        evaluate(p["run"] / "weights/best.pt", MAIN / "configs/crack_autodl.yaml", args.mode,
