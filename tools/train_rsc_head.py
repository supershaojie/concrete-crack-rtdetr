...
import torch
from init_rsc_head import ROOT, MODEL_DIR, VARIANTS, initialize, require, runtime, sha256, verify_model, write_json, is_added, BASE_COMMIT
...
MAIN = Path(os.environ.get("RSC_HEAD_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))
...
def record_source(folder, c2_args, data):
    ...
    subprocess.run(["git", "archive", "--format=tar.gz", "--output=" + str(folder / "source_snapshot.tar.gz"), "HEAD",
                    ...
                    "configs", "docs/rsc_head", "docs/rsc_head/c2_args.yaml", ".gitattributes"], cwd=ROOT, check=True)
...
def paths(variant):
    ...
    return dict(name=name, run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/rsc_head",
                init=ROOT / "weights" / "rsc_head_controlled_init.pt",
...
def recipe(c2_path, variant, init):
    source, expected = YAML.load(c2_path), YAML.load(ROOT / "docs/rsc_head/c2_args.yaml")
    ...
    for row in rows:
        if row["changed"]:
            row["reason"] = "experiment model/output identity" if row["field"] in {"model", "name", "save_dir"} else "explicit RSC_HEAD_MAIN root relocation; same data/recipe required"
    return target, rows


def verify_data_config(path):
    source, expected = YAML.load(path), YAML.load(ROOT / "docs/rsc_head/c2_data.yaml")
...
def verify_delivery():
    ...
    record = ROOT / "outputs/rsc_head_delivery.json"
    require(record.is_file(), "Run sync_rsc_head.sh with the delivered full SHA first")
    info = json.loads(record.read_text(encoding="utf-8"))
    head = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
    require(info["commit"] == head and info["branch"] == "codex/rsc-head", "Pinned delivery changed")
...
def duplicate_processes():
    ...
    for item in Path("/proc").glob("[0-9]*/cmdline"):
        try:
            ...
            if int(item.parent.name) != os.getpid() and any(v.endswith(b"train_rsc_head.py") for v in argv) and b"worker" in argv:
...
def start_direct(variant):
    ...
    require(not duplicate_processes(), "Existing RSC-Head worker process protected")
    require(not p["launch"].exists() and not p["init"].exists(), "Existing RSC-Head launch/init protected")
    require(ROOT.resolve() != MAIN.resolve(), "Use the independent RSC-Head worktree, not the main checkout")
    ...
    lock = p["run"].with_name(p["run"].name + ".rsc_head.lock")
    ...
    try:
        ...
        command = [sys.executable, "-u", str(ROOT / "tools/train_rsc_head.py"), "worker", variant]
        plan["command"] = command
        write_json(p["launch"] / "plan.json", plan)
        # An existing tmux server may have another experiment's environment.
        env = {"PYTHONPATH": str(ROOT / "ultralytics-main"), "YOLO_AUTOINSTALL": "false", "RSC_HEAD_MAIN": str(MAIN), "CONDA_DEFAULT_ENV": os.environ.get("CONDA_DEFAULT_ENV")}
...
def run_state(p):
    ...
    if launch.get("status") == "dispatched" and shutil.which("tmux"):
        if subprocess.run(["tmux", "has-session", "-t", "rsc-head-training"], capture_output=True).returncode:
...
def status(variant):
    ...
    if shutil.which("tmux"):
        active = subprocess.run(["tmux", "has-session", "-t", "rsc-head-training"], capture_output=True).returncode == 0
