"""EVC native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_evc import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_evc import MAIN, paths, run_state
...
def evaluate(weights, data, split, output, device="0", val_report=None):
    ...
    report = dict(status="failed", runtime=info, split=split, settings=settings, checkpoint=str(weights),
                  ...
                  boxes="native final Decoder layer (EVC cross-attention)", selection="Training val selects best; freeze checkpoint/config before independent test",
...
def validate_complete(p):
    ...
    require(all((launch / n).is_file() for n in required), "EVC direct-start metadata incomplete")
...
def package(destination=None):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/evc" / f"evc_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    ...
    logs = ROOT / "outputs/evc_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/evc/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/evc/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed EVC directories")
        p = paths("evc_deform")
