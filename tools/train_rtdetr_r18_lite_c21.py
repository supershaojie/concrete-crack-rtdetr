"""Prepare a field-locked C2 -> C21 launch plan. Training requires --execute or --tmux explicitly."""
...
from init_rtdetr_r18_lite_sala_controlled import (
    DEFAULT_OUTPUT, ROOT, SOURCE_SHA256, SALA_CFG, clean_checkpoint, require, runtime_info, sha256,
    verify_module, verify_protected, write_json, branch_modules,
)
...
DEFAULT_NAME = "c21_rtdetr_r18_lite_sala_e200_b16_onlineaug"
...
def build_locked_args(baseline: dict, initialized: Path, name: str) -> tuple[dict, list[dict]]:
    ...
    rows = [{"field": key, "C2": baseline[key], "C21": target[key], "equal": typed_equal(baseline[key], target[key]),
             "C2_type": type(baseline[key]).__name__, "C21_type": type(target[key]).__name__,
...
def prepare(args: argparse.Namespace) -> tuple[dict, Path]:
    ...
    require(args.initialized.is_file(), f"Missing clean SALA initialization: {args.initialized}")
    ...
    require(audit["runtime"]["torch_version"].split("+")[0] == "2.1.2", "Formal C21 launch requires actual PyTorch 2.1.2 audit.")
    ...
    require(checkpoint.get("sala_provenance", {}).get("source_sha256") == SOURCE_SHA256,
    ...
    plan = {
        ...
        "initialized_sha256": checkpoint_hash, "model_yaml": str(SALA_CFG),
...
def claim_launch(plan):
    ...
    lock = target.with_name(target.name + ".c21.launch.lock")
...
def launch_status(report_dir, name):
    ...
    lock = target.with_name(target.name + ".c21.launch.lock") if target else None
    marker = ROOT / "outputs/c21/prepare.state"
...
def assert_no_active_launch(plan):
    ...
    require(not state["tmux_active"] and not state["processes"], f"Existing C21 session/process: {state}")
...
def install_preflight(model, plan, report_dir):
    ...
    def before_first_batch(trainer):
        from audit_rtdetr_r18_lite_c21 import optimizer_rows, state_hashes
        actual_args_check(vars(trainer.args), plan["target_args"])
    ...
    def enable_allocation_logging(trainer):
        for module in branch_modules(trainer.model).values(): module.sala_stats_interval = 200

    def allocation_logging(trainer):
        rows = {}
        for name, module in branch_modules(trainer.model).items():
            if module.sala_last_stats is not None:
                rows[name] = module.sala_last_stats
                module.sala_last_stats = None
        if rows:
...
def main() -> None:
    ...
    parser.add_argument("--audit-report", type=Path, default=ROOT / "outputs/c21/audit.json")
    parser.add_argument("--name", default=DEFAULT_NAME)
    parser.add_argument("--report-dir", type=Path, default=ROOT / "outputs/c21/launch_c21")
    ...
    if args.tmux or args.execute:
        ...
        require(marker.is_file() and marker.read_text().strip() == "passed",
                "Latest prepare did not complete successfully; run bash tools/autodl_c21.sh prepare and inspect its error logs.")
    plan, _ = prepare(args)
    if args.tmux:
        require(os.name == "posix", "tmux launch is intended for the Linux server.")
        token = claim_launch(plan)
        session = f"c21_{args.name}"
        ...
        supervisor = [sys.executable, "-u", str(ROOT / "tools/c21_tmux_worker.py"),
                      str(args.report_dir.resolve()), *command]
        # An existing C15 tmux server may have a different environment; pin SALA import path explicitly.
        shell_command = shlex.join(["env", f"PYTHONPATH={ROOT / 'ultralytics-main'}", *supervisor])
        shell_command += "; rc=$?; printf '\\nC21 finished: exit=%s. Logs retained; press Enter to open shell.\\n' \"$rc\"; read -r _; exec bash -i"
        try:
