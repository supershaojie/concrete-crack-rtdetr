...
from init_rsc_head import ROOT, require, write_json
import train_rsc_head as train
...
def main():
    ...
    git('clone','--bare','--shared','--single-branch','--branch','codex/rsc-head',str(ROOT),str(remote))
    ...
    git('push','origin','HEAD:refs/heads/codex/rsc-head',cwd=main_repo)
...
if [[ "${3:-}" == fetch && "${4:-}" == origin ]]; then
 exec "$RSC_TEST_GIT" "$1" "$2" fetch "$RSC_TEST_REMOTE" "${@:5}"
fi
exec "$RSC_TEST_GIT" "$@"
''')
    if os.name != 'nt': (wrapper/'git').chmod(0o755)
    env=dict(os.environ, RSC_TEST_GIT=realgit, RSC_TEST_REMOTE=str(remote),RSC_TEST_WINDOWS=str(int(os.name=='nt')),
             RSC_HEAD_PYTHON=sys.executable,PATH=str(wrapper)+os.pathsep+os.environ['PATH'])
    checks=[]
    def sync(name, sha=pin, target=worktree, success=True):
        ...
        cp=subprocess.run([args.bash,'-c',shell,'rsc-sync-test',str(wrapper),str(ROOT/'tools/sync_rsc_head.sh'),sha,str(main_repo),str(target)],env=env,capture_output=True,text=True)
    ...
    require(git('rev-parse','FETCH_HEAD',cwd=main_repo)==advance, 'Fixture remote redirection/advancement not exercised')
    ...
    dirty=worktree/'uncommitted.txt';dirty.write_bytes(b'preserve fixture')
    sync('dirty_target_rejected',success=False)
    require(dirty.read_bytes()==b'preserve fixture','Dirty target overwritten')
