"""Audit C20 initialization, decoder behavior, dtypes and optional real-data smoke. No formal training."""
...
from init_rtdetr_r18_lite_c20 import (
    BASE_CFG, DEFAULT_SOURCE, DEFAULT_OUTPUT, ROOT, SOURCE_SHA256, new_prefix, C20_CFG,
    clean_checkpoint, read_source, require, runtime_info, sha256, tensor_rows, verify_module,
    verify_protected, verify_reloads, write_json, topology, remap_key, common_rows, new_state_keys, branch_modules,
)
...
def audit_optimizer(baseline, target):
    ...
    require({r["name"] for r in added} == {prefix + "." + n for prefix, module in branch_modules(target).items() for n, _ in module.named_parameters()},
...
def audit_initialization(source, initialized, recipe_path=None):
    ...
    for nc in (1, 80):
        ...
        torch.manual_seed(42)
        fresh_target = RTDETRDetectionModel(str(C20_CFG), nc=nc, verbose=False)
...
def audit_real_train_api(initialized, nc, expected, expected_rng, recipe_path=None):
    ...
    from train_rtdetr_r18_lite_c20 import actual_args_check, build_locked_args, DEFAULT_NAME
...
def audit_wiring(initialized):
    """Temporary observation only: prove CBR receives the final Neck P3 object."""
    model = RTDETR(str(initialized)).model.eval()
    verify_module(model)
    graph, captured, handles = topology(), {}, []
    def capture(name):
        def hook(module, inputs, output):
            captured[name] = output
        return hook
    for name, index in zip(("P3", "P4", "P5"), graph["decoder_inputs"]):
        handles.append(model.model[index].register_forward_hook(capture(name)))
    handles.append(model.model[graph["cscef"]].register_forward_hook(capture("lateral")))
    def observe(module, inputs):
        captured["CBR_P3"] = inputs[0]
    handles.append(model.model[-1].cbr.register_forward_pre_hook(observe))
    try:
        with torch.inference_mode():
            model(torch.rand(1, 3, 640, 640))
        require(captured["CBR_P3"] is captured["P3"] and captured["CBR_P3"] is not captured["lateral"],
                "CBR received an intermediate feature instead of final Neck P3.")
        shapes = {name: list(captured[name].shape) for name in ("P3", "P4", "P5")}
        require([v[-2:] for v in shapes.values()] == [[80, 80], [40, 40], [20, 20]], "Incorrect pyramid strides/order.")
        return {**graph, "shapes_640": shapes, "cbr_receives_final_p3_object": True,
                "observation_hooks_removed": True}
    finally:
...
def audit_precision(initialized, device, mode):
    ...
    if not half:
        ...
        for count in (0, 3, 3):
            ...
            grads = branch_gradients(model) if count else {
            ...
            steps.append({"gt_count": count, "loss": float(loss.detach()), "module_gradients": grads})
        ...
        buffer = io.BytesIO()
        torch.save(model, buffer); buffer.seek(0)
        restored = torch.load(buffer, weights_only=False).eval()
        verify_module(restored, require_zero=False)
        with torch.inference_mode():
            report["nonzero_fp32_reload"] = compare_output(model(image), restored(image))
            if device == "cuda":
                result = restored.half()(image.half())[0]
                require(torch.isfinite(result).all() and (result[..., 2:4] > 0).all(), "Nonzero half reload forward failed.")
                report["nonzero_half_reload_forward"] = "passed"
...
def benchmark(initialized, source, device):
    ...
    for label, w in (("C2", base_weights), ("C20", weights)):
...
def run_regressions(report_dir):
    result = {}
    for pattern in ("test_cbr.py", "test_cbr_tools.py", "test_c20.py"):
...
def main():
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "outputs/c20/audit.json")
    ...
    parser.add_argument("--smoke-dir", type=Path, default=ROOT / "outputs/c20/smoke")
    ...
    try:
        ...
        if args.smoke_data:
            from smoke_rtdetr_r18_lite_c20 import smoke
