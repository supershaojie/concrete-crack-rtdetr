...
from init_lcr import ROOT, require, runtime, sha256, write_json
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(prefix="lcr_ops_", dir=ROOT / "outputs") as d:
        temporary = Path(d)
        main, root = temporary / "main", temporary / "worktree"
        root.mkdir(); (main / "configs").mkdir(parents=True)
        (root / "docs/lcr").mkdir(parents=True)
        (root / "docs/lcr/c2_args.yaml").write_bytes((ROOT / "docs/lcr/c2_args.yaml").read_bytes())
        ...
        p = dict(name="fixture", run=main / "runs/c_series" / train.VARIANTS["lcr_aifi"][2], launch=root / "outputs/lcr", init=root / "init.pt", source=source,
                 c2_args=ROOT / "docs/lcr/c2_args.yaml")
        calls = []
        ...
        with ExitStack() as stack:
            ...
            report["duplicate_reservation_rejected"] = rejected(lambda: train.start_direct("lcr_aifi"))
