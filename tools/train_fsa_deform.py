...
import torch
from init_fsa_deform import ROOT, VARIANTS, initialize, require, runtime, sha256, verify_model, write_json, is_added, BASE_COMMIT
from ultralytics import RTDETR
from ultralytics.models.rtdetr.train import RTDETRTrainer
from ultralytics.utils import YAML, ASSETS
from ultralytics.data.utils import check_det_dataset

MAIN = Path(os.environ.get("FSA_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))
...
def record_source(folder, c2_args, data):
    ...
    subprocess.run(["git", "archive", "--format=tar.gz", "--output=" + str(folder / "source_snapshot.tar.gz"), "HEAD",
                    ...
                    "configs", "docs/fsa_deform", ".gitattributes"], cwd=ROOT, check=True)
...
def paths(variant):
    ...
    return dict(name=name, run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/fsa_deform",
...
def recipe(c2_path, variant, init):
    source, expected = YAML.load(c2_path), YAML.load(ROOT / "docs/fsa_deform/c2_args.yaml")
    ...
    for row in rows:
        if row["changed"]:
            row["reason"] = "experiment model/output identity" if row["field"] in {"model", "name", "save_dir"} else "explicit FSA_MAIN root relocation; same data/recipe required"
    return target, rows
...
def start_direct(variant):
    p = paths(variant)
    require(ROOT.resolve() != MAIN.resolve(), "Use the independent FSA worktree, not the main checkout")
    ...
    for proc in Path("/proc").glob("[0-9]*/cmdline"):
        try:
            commandline = proc.read_bytes().replace(b"\0", b" ").decode(errors="replace")
        except OSError:
            continue
        require(not ("train_fsa_deform.py worker fsa_deform" in commandline), "Same experiment worker already running")
    # Atomic, persistent reservation shared by all worktrees; preserve failure evidence too.
    lock = p["run"].with_name(p["run"].name + ".fsa_deform.lock")
    ...
    write_json(p["launch"] / "launch_state.json", dict(status="initializing", pid=os.getpid(), process_token=process_token(os.getpid()), full_server_preflight="NOT_RUN"))
    try:
        ...
        shutil.copyfile(ROOT / "ultralytics-main/ultralytics/cfg/models/rt-detr" / VARIANTS[variant][0], p["launch"] / "model.yaml")
        shutil.copyfile(ROOT / "docs/fsa_deform/validation.json", p["launch"] / "fsa_validation.json")
        ...
        command = [sys.executable, "-u", str(ROOT / "tools/train_fsa_deform.py"), "worker", variant]
        # An existing tmux server may have another experiment's environment.
        env = {"PYTHONPATH": str(ROOT / "ultralytics-main"), "YOLO_AUTOINSTALL": "false", "FSA_MAIN": str(MAIN), "CONDA_DEFAULT_ENV": os.environ.get("CONDA_DEFAULT_ENV")}
...
def run_state(p):
    ...
    if process:
        ...
        return "RUNNING" if token and token == process.get("process_token") else "FAILED"
    if launch.get("status") in {"dispatched", "initializing"}:
        session = launch.get("session", "fsa_deform-training")
        active = bool(shutil.which("tmux")) and subprocess.run(["tmux", "has-session", "-t", session], capture_output=True).returncode == 0
        if launch.get("status") == "initializing":
            token = process_token(launch.get("pid", -1))
            active = bool(token and token == launch.get("process_token"))
...
def status(variant):
    ...
    if shutil.which("tmux"):
        active = subprocess.run(["tmux", "has-session", "-t", "fsa_deform-training"], capture_output=True).returncode == 0
