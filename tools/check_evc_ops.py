...
from init_evc import ROOT, require, runtime, sha256, write_json
import train_evc as train
import evc_results as results
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(prefix="evc_ops_", dir=ROOT / "outputs") as d:
        temporary = Path(d)
        main, root = temporary / "main", temporary / "worktree"
        root.mkdir(); (main / "configs").mkdir(parents=True)
        (root / "docs/evc").mkdir(parents=True)
        (root / "docs/evc/c2_args.yaml").write_bytes((ROOT / "docs/evc/c2_args.yaml").read_bytes())
        ...
        p = dict(name="fixture", run=main / "runs/c_series" / train.VARIANTS["evc_deform"][2], launch=root / "outputs/evc", init=root / "init.pt", source=source,
                 c2_args=ROOT / "docs/evc/c2_args.yaml")
        calls = []
        ...
        with ExitStack() as stack:
            ...
            report["duplicate_reservation_rejected"] = rejected(lambda: train.start_direct("evc_deform"))
