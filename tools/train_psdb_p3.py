"""Pinned PSDB-P3 plan/start/resume/status; no automatic training from preflight."""
...
import torch
from init_psdb_p3 import (ROOT, VARIANTS, SOURCE_SHA256, BRANCH, require, sha256,
                         BASE_COMMIT, build_training_model, verify_model, is_added, git)
...
MAIN = Path(os.environ.get("PSDB_P3_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))
DEFAULT_VARIANT = "cbr_lif_psdb_p3_v1"
...
def code_identity():
    ...
    names = git("ls-files", "--cached", "--others", "--exclude-standard", "-z",
        "tools", "ultralytics-main/ultralytics", "configs", "docs/psdb_p3").split("\0")
    return {name: hashlib.sha256((ROOT / name).read_bytes().replace(b"\r\n", b"\n")).hexdigest()
            ...
            (Path(name).suffix in {".py", ".yaml", ".sh"} or
             name in {"docs/psdb_p3/parent_dataset_inventory.json", "docs/psdb_p3/parent_results.csv"})}


def paths(variant=DEFAULT_VARIANT):
    name = VARIANTS[variant][2]
    return dict(run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/psdb_p3" / variant,
                init=ROOT / "weights" / f"{variant}_controlled_init.pt",
                source=MAIN / "weights/rtdetr_r18_lite_imagenet_backbone_init.pt",
                data=MAIN / "configs/crack_autodl.yaml", preflight=ROOT / "outputs/psdb_p3" / variant / "preflight/checks.json")


def recipe(init, variant=DEFAULT_VARIANT, data=None):
    """Every field originates in the actual successful parent args snapshot."""
    original = YAML.load(ROOT / "docs/psdb_p3/parent_args.yaml")
...
def verify_data(data):
    data = Path(data)
    raw, expected = YAML.load(data), YAML.load(ROOT / "docs/psdb_p3/parent_data.yaml")
    ...
    require(resolved["nc"] == 1, "PSDB-P3 requires nc=1")
    ...
    parent = json.loads((ROOT / "docs/psdb_p3/parent_dataset_inventory.json").read_text(encoding="utf-8"))
...
def require_preflight(report_path, initialized, data, variant=DEFAULT_VARIANT):
    ...
    require(sha256(paths(variant)["source"]) == SOURCE_SHA256, "Current unified source hash mismatch")
...
def clean_code():
    require(not git("status", "--porcelain", "--untracked-files=no").strip(), "Commit source changes, then rerun preflight")
    tracked = set(git("ls-files").splitlines())
    require(set(code_identity()) <= tracked, "Untracked executable/configuration files must be committed before server preflight/start")
    require(os.environ.get("PSDB_P3_SHA") == head(), "Source this experiment environment with the delivered full SHA")


def verify_initialized(path, variant=DEFAULT_VARIANT):
    ...
    require(checkpoint.get("epoch") == -1 and all(checkpoint.get(k) is None for k in
            ...
            "Start requires a controlled, untrained checkpoint")
    provenance = checkpoint.get("psdb_p3_provenance", {})
    require(provenance.get("variant") == variant and provenance.get("source_sha256") == SOURCE_SHA256 and
            provenance.get("base_commit") == BASE_COMMIT and provenance.get("variants_new_exact") is True,
            "Controlled initialization provenance mismatch")
    verify_model(checkpoint["model"], variant, zero=True)
...
def make_plan(args):
    ...
    verify_initialized(initialized, args.variant)
    ...
    plan = dict(variant=args.variant, commit=head(), files=code_identity(), args=resolved,
                ...
                run=str(p["run"]), parent_args_sha256=sha256(ROOT / "docs/psdb_p3/parent_args.yaml"))
...
def validate_plan(plan):
    ...
    require(plan["parent_args_sha256"] == sha256(ROOT / "docs/psdb_p3/parent_args.yaml"), "Parent recipe changed")
...
def checkpoint_resume_contract(checkpoint, variant=DEFAULT_VARIANT):
    """Reject stripped/completed/init checkpoints before native resume is dispatched."""
    require(0 <= checkpoint.get("epoch", -1) < 199, "Only unfinished training epochs can resume")
    optimizer, scaler = checkpoint.get("optimizer"), checkpoint.get("scaler")
    require(isinstance(optimizer, dict) and bool(optimizer.get("state")), "Resume optimizer has no learned state")
    require(isinstance(scaler, dict) and float(scaler.get("scale", 0)) > 0, "Native AMP scaler state missing")
    require(checkpoint.get("ema") is not None and checkpoint.get("updates", 0) > 0, "Learned EMA state missing")
    saved = checkpoint["ema"]
    verify_model(saved, variant)
    require(torch.isfinite(saved.model[19].psdb.W_o.weight).all().item() and
            torch.count_nonzero(saved.model[19].psdb.W_o.weight).item() > 0, "Resume PSDB branch is unlearned/nonfinite")
    require(saved.model[-1].nc == 1, "Resume requires trained nc=1")
    args = checkpoint.get("train_args", {})
    require(args.get("epochs") == 200 and args.get("name") == VARIANTS[variant][2] and
            args.get("batch") == 16 and args.get("amp") is True, "Resume experiment identity/recipe mismatch")
...
def nested_state_equal(left, right):
    ...
    if isinstance(left, dict) and isinstance(right, dict):
        return left.keys() == right.keys() and all(nested_state_equal(left[k], right[k]) for k in left)
    if isinstance(left, (tuple, list)) and isinstance(right, (tuple, list)):
        return len(left) == len(right) and all(nested_state_equal(a, b) for a, b in zip(left, right))
    return left == right


def audit_native_resume(trainer, checkpoint, variant=DEFAULT_VARIANT):
    """Audit the actual trainer after native restoration, including quantized saved states."""
    checkpoint_resume_contract(checkpoint, variant)
    require(trainer.start_epoch == checkpoint["epoch"] + 1, "Native resume epoch mismatch")
    require(nested_state_equal(trainer.optimizer.state_dict(), checkpoint["optimizer"]), "Native optimizer state not restored")
    require(nested_state_equal(trainer.scaler.state_dict(), checkpoint["scaler"]), "Native AMP scaler state not restored")
    expected = checkpoint["ema"].float().state_dict()
    require(all(torch.equal(value.detach().cpu(), trainer.ema.ema.state_dict()[key].detach().cpu())
                for key, value in expected.items()), "Native EMA state not restored")
    require(trainer.ema.updates == checkpoint["updates"], "Native EMA update count not restored")
    current = trainer.model.state_dict()
    require(all(torch.equal(value.detach().cpu(), current[key].detach().cpu())
                for key, value in expected.items() if is_added(key)), "Native learned PSDB state not restored")
    return dict(status="PASSED", start_epoch=trainer.start_epoch, optimizer_exact=True, scaler_exact=True,
                ema_exact=True, learned_psdb_exact=True, ema_updates=trainer.ema.updates,
                note="Compared to actual native checkpoint; native EMA and optimizer serialization may use fp16")
...
def dispatch(args, resume=False):
    ...
    env_file = ROOT / "outputs/psdb_p3.env"
    require(env_file.is_file(), "Run sync_psdb_p3.sh to create this worktree environment")
    state = run_state(p)
    ...
    command = [sys.executable, "-u", str(ROOT / "tools/train_psdb_p3.py"), "worker", "--variant", args.variant, "--attempt", str(attempt)]
    ...
    try:
        ...
        class RecordingTrainer(RTDETRTrainer):
            ...
            def resume_training(self, checkpoint):
                ...
                if self.resume:
                    write_json(attempt / "native_resume.json", audit_native_resume(self, checkpoint, args.variant))
        ...
        def setup(trainer):
            ...
            trainable = {n: ids.count(id(v)) for n, v in trainer.model.named_parameters() if v.requires_grad}
            added = {n: ids.count(id(v)) for n, v in trainer.model.named_parameters() if is_added(n)}
            require(len(ids) == len(trainable) and all(v == 1 for v in trainable.values()),
                    "Optimizer must cover every trainable parameter exactly once, without extras")
            ...
            write_json(attempt / "training_setup.json", dict(amp=True, start_epoch=trainer.start_epoch,
                       all_trainable_optimizer_occurrences=trainable, new_optimizer_occurrences=added,
        ...
        def epoch_record(trainer):
            ...
            save_state()
            if epoch in {40, 80}:
                reference = next((r for r in read_epoch_rows(ROOT / "docs/psdb_p3/parent_results.csv") if int(r["epoch"]) == epoch), None)
