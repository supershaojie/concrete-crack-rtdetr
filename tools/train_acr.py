"""Prepare a field-locked C2 -> ACR launch plan. Training requires --execute or --tmux explicitly."""
...
def typed_equal(a, b):
    """Numeric/bool coercion must never disguise a changed C2 field type."""
...
def build_locked_args(baseline: dict, initialized: Path, name: str) -> tuple[dict, list[dict]]:
    ...
    reference = YAML.load(ROOT / "ultralytics-main/tests/fixtures/c2_original_args.yaml")
    differences = [key for key in reference if not typed_equal(baseline.get(key), reference[key])]
    require(not differences, f"Authoritative C2 file differs from the verified original values/types: {differences}")
    for key, expected in C2_KEY_FIELDS.items():
        actual = baseline.get(key)
        require(typed_equal(actual, expected), f"C2 sanity check failed: {key}={actual!r}, expected {expected!r}.")
    ...
    rows = [{"field": key, "C2": baseline[key], "ACR": target[key], "equal": typed_equal(baseline[key], target[key]),
             "C2_type": type(baseline[key]).__name__, "ACR_type": type(target[key]).__name__,
...
def prepare(args: argparse.Namespace) -> tuple[dict, Path]:
    ...
    verify_protected()
    target, rows = build_locked_args(baseline, args.initialized, args.name)
    ...
    require(args.initialized.is_file(), f"Missing clean ACR initialization: {args.initialized}")
    ...
    require(audit.get("status") == "passed",
            f"A passing CPU and CUDA audit is required; status={audit.get('status')}, error={audit.get('error')}.")
    require(audit.get("cuda", {}).get("status") == "passed", "CUDA has not passed.")
    smoke = audit.get("real_data_smoke", {})
    require(audit.get("native_amp", {}).get("status") == "passed", "Native AMP helper did not pass; inspect resources/dependencies.")
    require(smoke.get("status") == "passed" and smoke.get("amp") is True,
            "Run the server audit with --smoke-data before preparing formal training.")
    require(smoke.get("data_sha256") == sha256(Path(target["data"]))
            and smoke.get("c2_args_sha256") == sha256(args.c2_args), "Smoke used a different dataset/C2 recipe.")
    require(audit["runtime"]["torch_version"].split("+")[0] == "2.1.2", "Formal ACR launch requires actual PyTorch 2.1.2 audit.")
    ...
    require(audit.get("post_smoke_formal_initialization", {}).get("status") == "passed", "Missing post-smoke clean initialization check.")
    initialization_report = json.loads((args.report_dir / "initialization.json").read_text(encoding="utf-8"))
    require(initialization_report.get("status") == "passed" and initialization_report.get("output_sha256") == checkpoint_hash
            and initialization_report.get("source_sha256") == SOURCE_SHA256, "Initialization report does not match this checkpoint/source.")
    runtime = runtime_info()
    require(not runtime["git_status"], "Formal preparation requires a clean committed worktree.")
    require(audit["runtime"]["git_commit"] == runtime["git_commit"], "Commit changed since audit; rerun it.")
    ...
    require(checkpoint.get("acr_provenance", {}).get("variant") == VARIANT, "Wrong ACR variant initialization")
    ...
    plan = {
        ...
        "status": "prepared_no_training", "variant": VARIANT, "runtime": runtime, "c2_args": str(args.c2_args.resolve()),
        "c2_args_sha256": sha256(args.c2_args), "initialized": str(args.initialized.resolve()),
        "initialized_sha256": checkpoint_hash, "model_yaml": str(ACR_CFG),
    ...
    if plan_path.exists():
        ...
        # A newly executed, hash-validated audit may refresh an unlaunched plan.
        for key in ("target_args", "c2_args_sha256", "initialized_sha256", "data_config_sha256"):
            require(typed_equal(previous[key], plan[key]), f"Existing preparation differs ({key}); use a fresh --report-dir.")
...
def recheck(plan):
    ...
    require(typed_equal(rebuilt, plan["target_args"]), "Prepared recipe differs from original C2.")
...
def claim_launch(plan):
    ...
    recheck(plan)
    assert_no_active_launch(plan)
    if VARIANT == "c23":
        review = combination_review()
        write_json(Path(plan["report_dir"]) / "combination_review.json", review)
...
def combination_review():
    """Explicit later decision; preparing C23 does not authorize its formal experiment."""
    require(os.environ.get("ACR_COMBINATION_REVIEWED") == "1",
            "Review full corrected C2/C17/C22 val before C23 start; set ACR_COMBINATION_REVIEWED=1 only after that decision.")
    paths = {"C22": ROOT / "outputs/c22/evaluation_val/metrics_summary.json"}
    for label in ("C2", "C17"):
        value = os.environ.get(f"ACR_{label}_VAL_REPORT")
        require(value, f"Set ACR_{label}_VAL_REPORT to its completed corrected val report")
        paths[label] = Path(value)
    reports = {k: json.loads(p.read_text(encoding="utf-8")) for k,p in paths.items()}
    reference = reports['C22']
    for label,r in reports.items():
        require(r['status']=='completed' and r['split']=='val' and r['postprocess_policy']=='corrected', f"Invalid {label} val evidence")
        require(r['data_sha256']==reference['data_sha256'] and r['images']==reference['images'], "Val splits differ")
        for key in ('imgsz','batch','half','conf','iou','max_det','augment','rect'):
            require(r['requested_args'][key]==reference['requested_args'][key], "Val settings differ")
    require(reference['mAP50_95'] > reports['C2']['mAP50_95'], "C22 full val does not support independent mAP50-95 benefit")
    return {'reviewed_by_user':True,'reports':{k:{'path':str(p),'sha256':sha256(p),'mAP50_95':reports[k]['mAP50_95']} for k,p in paths.items()},
            'note':'Single full-val comparison supports scheduling, not statistical significance or a guarantee of combination gains.'}


def launch_status(report_dir, name):
    """Read run, lock, tmux, process and terminal exit evidence without preparing/training."""
    import psutil
    ...
    for process in psutil.process_iter(["pid", "cmdline"]):
        try:
            ...
            if process.pid != os.getpid() and any("--run-plan" == part for part in command):
                if str(report_dir / "launch_plan.json") in command:
    ...
    return {"prepare_state": marker.read_text().strip() if marker.is_file() else "not_run",
            ...
            "console": str(report_dir / "console.log"), "bootstrap": str(report_dir / "bootstrap.log")}


def assert_no_active_launch(plan):
    state = launch_status(Path(plan["report_dir"]), plan["target_args"]["name"])
    require(not state["tmux_active"] and not state["processes"], f"Existing ACR session/process: {state}")
    require(not any(k in state["records"] for k in ("exit_code.json", "process_exit_code.json", "tmux.json")),
            "Prior dispatch/exit evidence exists; preserve it and use a new run/report directory.")
...
def actual_args_check(actual, expected):
    ...
    differences = {k: {"expected": expected[k], "actual": actual[k], "expected_type": type(expected[k]).__name__,
                       "actual_type": type(actual[k]).__name__}
                   for k in expected if not typed_equal(actual[k], expected[k])}
...
def install_preflight(model, plan, report_dir):
    ...
    def before_first_batch(trainer):
        from audit_acr import optimizer_rows, state_hashes
        actual_args_check(vars(trainer.args), plan["target_args"])
    ...
    def allocation_logging(trainer):
        rows = {}
        for name, module in branch_modules(trainer.model).items():
            if module.acr_last_stats is not None:
                rows[name] = module.acr_last_stats
                module.acr_last_stats = None
        if rows:
            with (report_dir / 'allocation.jsonl').open('a', encoding='utf-8') as file:
...
def main() -> None:
    ...
    if args.tmux:
        ...
        supervisor = [sys.executable, "-u", str(ROOT / "tools/acr_tmux_worker.py"),
                      str(args.report_dir.resolve()), *command]
        # An existing C15 tmux server may have a different environment; pin ACR import path explicitly.
        shell_command = shlex.join(["env", f"PYTHONPATH={ROOT / 'ultralytics-main'}", f"ACR_VARIANT={VARIANT}", "YOLO_AUTOINSTALL=false", *supervisor])
        shell_command += "; rc=$?; printf '\\nACR finished: exit=%s. Logs retained; press Enter to open shell.\\n' \"$rc\"; read -r _; exec bash -i"
        try:
        ...
        print(f"Started tmux session: {session}\nAttach: tmux attach -t {shlex.quote(session)}")
        print(f"Console: {args.report_dir.resolve() / 'console.log'}\nExit status: {args.report_dir.resolve() / 'exit_code.json'}")
    elif args.execute:
