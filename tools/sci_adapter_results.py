"""SCI Adapter native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from sci_adapter import ROOT, require, runtime, sha256, topology, write_json, SOURCE_SHA256, VARIANTS, git
from ultralytics.utils import ops
from train_sci_adapter import MAIN, paths, run_state
...
def evaluate(variant, weights, data, split, output, device="0", val_report=None):
    ...
    report = dict(status="failed", runtime=info, split=split, settings=settings, checkpoint=str(weights),
                  ...
                  variant=variant, boxes="final decoder boxes; original C19 CBR once when variant enables CBR", selection="Training val selects best; freeze checkpoint/config before independent test",
...
def validate_complete(p):
    ...
    required = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml", "train_args.yaml",
                ...
                "source_from_base.patch", "source_record.json", "plan.json", "process.json", "console.log", "worker.sh", "launch_state.json", "model.yaml", "preflight.json", "worker_complete.json", "sci_adapter_stats.jsonl", "source_audit.json")
    require(all((launch / n).is_file() for n in required), "SCI Adapter direct-start metadata incomplete")
    ...
    diagnostics=[json.loads(line) for line in (launch/'sci_adapter_stats.jsonl').read_text(encoding='utf-8').splitlines()]
    require(diagnostics and all(np.isfinite(r['residual_to_Y4_RMS']) for r in diagnostics),'SCI diagnostics missing/nonfinite')
...
def missing_evidence(p):
    ...
    required_launch = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml",
                       ...
                       "process_exit_code.txt", "amp_resources.json", "model.yaml", "preflight.json", "worker_complete.json", "sci_adapter_stats.jsonl", "source_audit.json")
...
def package(variant, destination=None, allow_incomplete=False):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/sci_adapter" / variant / f"{variant}_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    ...
    logs = ROOT / "outputs/sci_adapter_command_logs" / variant
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/sci_adapter/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/sci_adapter/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed SCI Adapter directories")
