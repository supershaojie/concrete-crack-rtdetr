...
from init_rsc_head import ROOT, require, runtime, sha256, write_json
import train_rsc_head as train
import rsc_head_results as results
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(prefix="rsc_head_ops_", dir=ROOT / "outputs") as d:
        temporary = Path(d)
        main, root = temporary / "main", temporary / "worktree"
        root.mkdir(); (main / "configs").mkdir(parents=True)
        (root / "docs/rsc_head").mkdir(parents=True)
        (root / "docs/rsc_head/c2_args.yaml").write_bytes((ROOT / "docs/rsc_head/c2_args.yaml").read_bytes())
        ...
        p = dict(name="fixture", run=main / "runs/c_series" / train.VARIANTS["rsc_head"][2], launch=root / "outputs/rsc_head", init=root / "init.pt", source=source,
                 c2_args=ROOT / "docs/rsc_head/c2_args.yaml")
        calls = []
        ...
        with ExitStack() as stack:
            ...
            report["duplicate_reservation_rejected"] = rejected(lambda: train.start_direct("rsc_head"))
        ...
        states.append(train.run_state(q))
        require(states == ["NOT_STARTED", "RUNNING", "FAILED_INTERRUPTED", "FAILED_MISSING_OUTPUT", "SUCCEEDED"], "State classification")
