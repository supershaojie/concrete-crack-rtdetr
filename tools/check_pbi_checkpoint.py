"""CPU-only toy differential fixture for the PBI optimizer checkpoint policy.
...
from pbi_checkpoint import (PBICheckpointTrainer, OPTIMIZER_POLICY, native_serializer_identity,
...
def check():
    ...
    report = {"status": "RUNNING", "python": sys.version, "torch": str(torch.__version__), "device": "cpu",
              ...
              "source_sha256": {name: hashlib.sha256((ROOT / "tools" / name).read_bytes()).hexdigest()
                                for name in ("pbi_checkpoint.py", "check_pbi_checkpoint.py")}}
    with tempfile.TemporaryDirectory(prefix="pbi_checkpoint_fixture_") as temporary:
        ...
        pbi = build_fixture(PBICheckpointTrainer, folder / "pbi", model, optimizer, ema, scaler)
        native.save_model()
        pbi.save_model()
        original = torch.load(native.last, map_location="cpu", weights_only=False)
        updated = torch.load(pbi.last, map_location="cpu", weights_only=False)
        assert set(updated) == set(original) | {"pbi_checkpoint"}
        ...
        assert updated["pbi_checkpoint"]["optimizer_param_names"] == names
        assert updated["pbi_checkpoint"]["policy"] == OPTIMIZER_POLICY
        compare_exact({key: updated["pbi_checkpoint"][key] for key in native_serializer_identity()}, native_serializer_identity())
        ...
        half_mislabeled["pbi_checkpoint"] = deepcopy(updated["pbi_checkpoint"])
        ...
        for case in ("missing_name", "duplicate_name", "duplicate_param_id", "unknown_state_id", "missing_optimizer"):
            ...
            if case == "missing_name":
                broken["pbi_checkpoint"]["optimizer_param_names"][0].pop()
            elif case == "duplicate_name":
                broken["pbi_checkpoint"]["optimizer_param_names"][0][1] = broken["pbi_checkpoint"]["optimizer_param_names"][0][0]
        ...
        for key, values in active_before["state"].items():
            ...
            moments.append({"parameter_id": key, "nonzero_live": int(before.count_nonzero()),
                            ...
                            "nonzero_pbi_fp32": int(current.count_nonzero()),
                            "native_dtype": str(prior.dtype), "pbi_dtype": str(current.dtype),
        ...
        assert all(row["nonzero_live"] == row["nonzero_pbi_fp32"] for row in moments)
        # Last/best/periodic write routing and identical serialized byte reuse.
        saved = pbi.last.read_bytes()
        assert saved == pbi.best.read_bytes() == (pbi.wdir / "epoch2.pt").read_bytes()
        pbi.epoch, pbi.fitness, pbi.save_period = 3, .01, -1
        pbi.save_model()
        assert pbi.best.read_bytes() == saved and not (pbi.wdir / "epoch3.pt").exists()
        assert torch.load(pbi.last, map_location="cpu", weights_only=False)["epoch"] == 3
        ...
        bad = build_fixture(PBICheckpointTrainer, folder / "bad", bad_model, bad_optimizer, ModelEMA(bad_model), scaler)
        try:
            bad.save_model()
        except RuntimeError as error:
        ...
        report.update(status="PASSED", native_fields=sorted(original), extra_fields=["pbi_checkpoint"],
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "outputs/pbi/checkpoint_policy_fixture.json")
