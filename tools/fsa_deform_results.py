"""FSA native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_fsa_deform import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_fsa_deform import MAIN, paths, run_state
from ultralytics import RTDETR
from ultralytics.models.rtdetr.val import RTDETRValidator
from ultralytics.utils import YAML
from ultralytics.data.utils import check_det_dataset

POLICY = "C2_67c3078_native_RTDETRValidator_unchanged"
EVAL = dict(imgsz=640, batch=16, workers=8, half=False, conf=.001, iou=.7, max_det=300, augment=False, rect=False)


def native_predictions(preds, imgsz, conf):
    """Call unchanged C2 postprocess for metrics; export all queries separately without mutating its selection."""
    raw = preds[0] if isinstance(preds, (list, tuple)) else preds
    native = object.__new__(RTDETRValidator)
    native.args = SimpleNamespace(imgsz=imgsz, conf=conf)
    selected = native.postprocess(raw.clone())
    native.args.conf = -float("inf")
    full = native.postprocess(raw.clone())
    affected = 0
    for selected_image, all_queries in zip(selected, full):
        used = torch.zeros_like(all_queries["conf"], dtype=torch.bool)
        if len(selected_image["conf"]):
            used = ((all_queries["bboxes"][:, None] == selected_image["bboxes"][None]).all(-1) &
                    (all_queries["conf"][:, None] == selected_image["conf"][None])).any(-1)
        all_queries["used_for_metrics"] = used
        affected += int(not torch.equal(used, all_queries["conf"] > conf))
...
def evaluate(weights, data, split, output, device="0", val_report=None):
    ...
    report = dict(status="failed", runtime=info, split=split, settings=settings, checkpoint=str(weights),
                  ...
                  boxes="native final layer2 output; FSA cross-attention only in middle layer1", selection="Training val selects best; freeze checkpoint/config before independent test",
...
def validate_complete(p):
    ...
    required = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml", "train_args.yaml",
                ...
                "source_from_base.patch", "source_record.json", "plan.json", "process.json", "console.log", "worker.sh", "launch_state.json", "model.yaml", "fsa_validation.json")
    require(all((launch / n).is_file() for n in required), "FSA direct-start metadata incomplete")
...
def missing_evidence(p):
    ...
    required_launch = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml",
                       ...
                       "process_exit_code.txt", "amp_resources.json", "model.yaml", "fsa_validation.json")
...
def package(destination=None, allow_incomplete=False):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/fsa_deform" / f"fsa_deform_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    ...
    logs = ROOT / "outputs/fsa_deform_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/fsa_deform/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/fsa_deform/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed FSA directories")
        p = paths("fsa_deform")
