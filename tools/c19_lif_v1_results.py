"""C19 + LIF-Down native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_c19_lif_v1 import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_c19_lif_v1 import MAIN, paths, run_state
...
def evaluate(weights, data, split, output, device="0", val_report=None, evidence_scope="full_split"):
    ...
    report = dict(status="failed", evidence_scope=evidence_scope, runtime=info, split=split, settings=settings, checkpoint=str(weights),
                  ...
                  boxes="original C19 refined final Decoder layer", precision_recall_policy="each model own maximum-F1 working point", selection="Training val selects best; freeze checkpoint/config before independent test",
...
def validate_complete(p):
    ...
    require(all((launch / n).is_file() for n in required), "C19 + LIF-Down direct-start metadata incomplete")
    ...
    require(plan["full_server_preflight"] == "bounded_required", "Unexpected direct-start preflight record")
    require(plan['bounded_preflight']['status'] == 'PASSED' and
            sha256(launch/'preflight.json') == plan['bounded_preflight']['report_sha256'], 'Missing/changed bounded preflight')
    preflight=json.loads((launch/'preflight.json').read_text(encoding='utf-8'))
    require(preflight['status'] == 'PASSED' and preflight['capacity']['status'] == 'PASSED' and
            preflight['capacity']['batch'] == 16 and preflight['capacity']['imgsz'] == 640, 'Wrong capacity evidence')
    ...
    for split in ("val", "test"):
        ...
        inventory=json.loads((launch/'dataset_inventory.json').read_text(encoding='utf-8'))[split]
        require(report['split_paths_sha256'] == inventory['split_paths_sha256'] and report['images'] == inventory['images'] and
                report['ground_truth'] == inventory['boxes'], 'Evaluation differs from frozen split inventory')
...
def missing_evidence(p):
    missing = []
    for name in ("checks.json", "initialization.json", "ops_checks.json", "VALIDATION.md", "PROVENANCE.json", "parent_evidence.json"):
        if not (ROOT / "docs/c19_lif_v1" / name).is_file(): missing.append("metadata/source/docs/c19_lif_v1/" + name)
    required_launch = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml",
                       ...
                       "process_exit_code.txt", "amp_resources.json", "preflight.json", "preflight.log", "dataset_inventory.json")
...
def package(destination=None):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/c19_lif_v1" / f"c19_lif_v1_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
        ...
        for f in sorted(folder.rglob("*")):
            ...
            if f.is_file() and not any(x in f.relative_to(folder).parts for x in excluded):
                ...
                names = {"best.pt", "last.pt", "args.yaml", "results.csv", "results.png", "metrics.json",
                         ...
                         "launch_state.json", "exit_code.json", "process_exit_code.txt", "amp_resources.json", "training_state.json", "preflight.json", "preflight.log", "dataset_inventory.json", "warmup_batch_0.json", "warmup_batch_1.json", "warmup_batch_2.json"}
    ...
    tree(run, "training")
    if (ROOT / "outputs/c19_lif_v1_delivery.json").is_file():
        files["metadata/c19_lif_v1_delivery.json"] = ROOT / "outputs/c19_lif_v1_delivery.json"
    tree(launch, "metadata/launch", ("evaluation_val", "evaluation_test"))
    for split in ("val", "test"):
        tree(launch / ("evaluation_" + split), split)
    logs = ROOT / "outputs/c19_lif_v1_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/c19_lif_v1/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/c19_lif_v1/c2_args.yaml") or
    ...
    metadata = dict(created=datetime.now(timezone.utc).isoformat(), runtime=runtime(), evaluations=evaluations,
                    ...
                    unified_init_sha256=SOURCE_SHA256, full_server_preflight="bounded_required",
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed C19 + LIF-Down directories")
        p = paths("c19_lif_v1")
        ...
        resolved=check_det_dataset(str(p['launch']/'data_config.yaml'),autodownload=False)
        require(dataset_inventory(Path(resolved['path'])) == json.loads((p['launch']/'dataset_inventory.json').read_text(encoding='utf-8')),
                'Dataset paths/labels differ from training preflight')
