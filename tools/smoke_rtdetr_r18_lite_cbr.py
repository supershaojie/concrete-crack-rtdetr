...
from init_rtdetr_r18_lite_cbr_controlled import require, sha256, write_json, verify_module
...
def smoke(initialized, data, c2_args, output, expected_states):
    from audit_rtdetr_r18_lite_cbr import state_hashes, gradient_magnitudes, optimizer_rows, compare_output
    from train_rtdetr_r18_lite_cbr import build_locked_args, DEFAULT_NAME
    ...
    class SmokeTrainer(RTDETRTrainer):
        def _do_train(self):
            ...
            for step in range(3):
                ...
                grads = gradient_magnitudes(self.model.model[-1].cbr)
                ...
                report["steps"].append({"step": step, "loss": float(loss.detach()), "gt_count": count,
                                        "dn_num_split": preds[4]["dn_num_split"], "cbr_gradients": grads,
