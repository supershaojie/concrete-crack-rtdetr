...
from init_pbi import require, write_json, sha256, native_rebuild
...
def checkpoint_expected_state(checkpoint, amp):
    ...
    names = checkpoint["pbi_checkpoint"]["optimizer_param_names"]
...
def make_audit_trainer(model, amp, device, trainer_cls=None):
    if trainer_cls is None:
        ...
        trainer_cls = PBICheckpointTrainer
...
def step_once(trainer, batch, amp, gradients=False):
    ...
    row = dict(loss=float(loss.detach()), scale_before=scale, scale_after=float(trainer.scaler.get_scale()),
               ...
               pbi_grad_norms={name: _finite_norm(value) for name, value in captured.items() if ".pbi." in name},
...
def bounded_updates(trainer, batch, amp, max_batches=16, target_updates=3):
    ...
    if hasattr(trainer.model.model[17], "pbi"):
        require(torch.count_nonzero(trainer.model.model[17].pbi.Wo.weight) > 0, "PBI Wo did not learn")
        require(all(value is not None and value > 0 for value in rows[-1]["pbi_grad_norms"].values()),
                "PBI upstream gradients not activated")
...
def audit_checkpoint_resume(trainer, batch, amp, folder, trainer_cls=None):
    ...
    """
    from pbi_checkpoint import PBICheckpointTrainer, optimizer_param_names, require_checkpoint_policy
    from pbi_acceptance import CONTRACT_VERSION, evaluate_checkpoint
    trainer_cls = trainer_cls or PBICheckpointTrainer
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="_resume_audit_", dir=str(folder)) as temporary:
            ...
            report["serializer"] = deepcopy(checkpoint.get("pbi_checkpoint", {}))
            ...
            restoration["live_direct_independence"] = assert_no_shared_storage(trainer, direct)
            restoration["live_resumed_independence"] = assert_no_shared_storage(trainer, resumed)
            ...
            report["trajectory_repeatability"] = dict(status="PENDING", reason="Requires matching parent/PBI live controls")
