"""LIF-Down native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_lif_down import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_lif_down import MAIN, paths, run_state
...
def validate_complete(p):
    ...
    require(all((launch / n).is_file() for n in required), "LIF-Down direct-start metadata incomplete")
...
def missing_evidence(p):
    ...
    for name in ("checks.json", "initialization_mapping.json", "ops_checks.json", "validation.md"):
        if not (ROOT / "docs/lif_down" / name).is_file(): missing.append("metadata/source/docs/lif_down/" + name)
...
def package(destination=None):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/lif_down" / f"lif_down_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
        ...
        for f in sorted(folder.rglob("*")):
            ...
            if f.is_file() and not any(x in f.relative_to(folder).parts for x in excluded):
                ...
                names = {"best.pt", "last.pt", "args.yaml", "results.csv", "results.png", "metrics.json",
                         ...
                         "launch_state.json", "exit_code.json", "process_exit_code.txt", "amp_resources.json", "training_state.json"}
    ...
    tree(run, "training")
    if (ROOT / "outputs/lif_down_delivery.json").is_file():
        files["metadata/lif_down_delivery.json"] = ROOT / "outputs/lif_down_delivery.json"
    tree(launch, "metadata/launch", ("evaluation_val", "evaluation_test"))
    for split in ("val", "test"):
        tree(launch / ("evaluation_" + split), split)
    logs = ROOT / "outputs/lif_down_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/lif_down/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/lif_down/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed LIF-Down directories")
        p = paths("lif_down")
