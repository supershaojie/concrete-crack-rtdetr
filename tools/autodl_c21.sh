...
NAME=c21_rtdetr_r18_lite_sala_e200_b16_onlineaug
REPORT="$ROOT/outputs/c21/launch_c21"
INIT="$ROOT/weights/rtdetr_r18_lite_sala_imagenet_backbone_init.pt"
...
mkdir -p "$ROOT/outputs/c21"
exec > >(tee -a "$ROOT/outputs/c21/${MODE}_${STAMP}.bootstrap.log") 2>&1
trap 'rc=$?; printf "C21 %s FAILED: exit=%s line=%s command=%s\n" "$MODE" "$rc" "$LINENO" "$BASH_COMMAND" >&2; exit "$rc"' ERR
if [[ "$MODE" == prepare ]]; then printf 'running\n' > "$ROOT/outputs/c21/prepare.state"; fi

case "$MODE" in prepare|start|status|val|test|pack) ;; *) echo 'Usage: bash tools/autodl_c21.sh prepare|start|status|val|test|pack'; exit 2 ;; esac
...
# Keep preparation/evaluation from silently installing dependencies into the shared environment.
...
assert p.is_relative_to(pathlib.Path.cwd() / 'ultralytics-main'), 'Wrong Ultralytics import: expected C21 worktree'
...
case "$MODE" in
    prepare)
        if [[ -n "$(git status --porcelain)" ]]; then echo 'C21 worktree must be clean and committed.' >&2; exit 1; fi
        if [[ -e "$RUN" || -e "$RUN.c21.launch.lock" || -e "$REPORT/tmux.json" || -e "$REPORT/exit_code.json" ]]; then
            echo 'Prior C21 launch/results exist. Preserve them; use status. prepare cannot overwrite launch evidence.' >&2; exit 1
        fi
        ...
        if [[ ! -f "$INIT" ]]; then
            python tools/init_rtdetr_r18_lite_sala_controlled.py --source "$SOURCE" --output "$INIT" \
        ...
        python tools/audit_rtdetr_r18_lite_c21.py --source "$SOURCE" --initialized "$INIT" \
            --report "$AUDIT" --require-torch 2.1.2 --require-cuda --c2-args "$C2_ARGS" \
            --smoke-data "$DATA" --smoke-dir "$ROOT/outputs/c21/smoke_$STAMP" \
            2>&1 | tee "$REPORT/audit_$STAMP.console.log"
        cp "$AUDIT" "$REPORT/audit.json"
        python tools/train_rtdetr_r18_lite_c21.py --c2-args "$C2_ARGS" --initialized "$INIT" \
            --audit-report "$REPORT/audit.json" --name "$NAME" --report-dir "$REPORT"
        printf 'passed\n' > "$ROOT/outputs/c21/prepare.state"
    ...
    start)
        ...
        ;;
    status)
        python tools/train_rtdetr_r18_lite_c21.py --name "$NAME" --report-dir "$REPORT" --status
        ;;
    val|test)
...
PY
        EXTRA=()
        if [[ "$MODE" == test ]]; then EXTRA=(--val-report "$ROOT/outputs/c21/evaluation_val/metrics_summary.json"); fi
        python tools/c21_results.py evaluate --weights "$RUN/weights/best.pt" --data "$DATA" \
            --split "$MODE" --device 0 --batch 16 --output "$ROOT/outputs/c21/evaluation_$MODE" "${EXTRA[@]}"
        ;;
    pack)
        python tools/c21_results.py pack --run "$RUN" --launch "$REPORT" \
            --val "$ROOT/outputs/c21/evaluation_val" --test "$ROOT/outputs/c21/evaluation_test" \
            --output "$ROOT/outputs/c21/c21_sala_small.tar.gz"
        ;;
esac
