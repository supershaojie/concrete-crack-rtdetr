...
from init_lif_down import ROOT, require, runtime, sha256, write_json
import train_lif_down as train
import lif_down_results as results
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(prefix="lif_down_ops_", dir=ROOT / "outputs") as d:
        temporary = Path(d)
        main, root = temporary / "main", temporary / "worktree"
        root.mkdir(); (main / "configs").mkdir(parents=True)
        (root / "docs/lif_down").mkdir(parents=True)
        (root / "docs/lif_down/c2_args.yaml").write_bytes((ROOT / "docs/lif_down/c2_args.yaml").read_bytes())
        ...
        p = dict(name="fixture", run=main / "runs/c_series" / train.VARIANTS["lif_down"][2], launch=root / "outputs/lif_down", init=root / "init.pt", source=source,
                 c2_args=ROOT / "docs/lif_down/c2_args.yaml")
        calls = []
        ...
        with ExitStack() as stack:
            ...
            report["duplicate_reservation_rejected"] = rejected(lambda: train.start_direct("lif_down"))
