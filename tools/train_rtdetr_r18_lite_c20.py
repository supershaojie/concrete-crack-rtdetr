"""Prepare a field-locked C2 -> C20 launch plan. Training requires --execute or --tmux explicitly."""
...
from init_rtdetr_r18_lite_c20 import (
    DEFAULT_OUTPUT, ROOT, SOURCE_SHA256, C20_CFG, clean_checkpoint, require, runtime_info, sha256,
...
DEFAULT_NAME = "c20_rtdetr_r18_lite_cscef_cbr_e200_b16_onlineaug"
...
def build_locked_args(baseline: dict, initialized: Path, name: str) -> tuple[dict, list[dict]]:
    ...
    rows = [{"field": key, "C2": baseline[key], "C20": target[key], "equal": typed_equal(baseline[key], target[key]),
             "C2_type": type(baseline[key]).__name__, "C20_type": type(target[key]).__name__,
...
def prepare(args: argparse.Namespace) -> tuple[dict, Path]:
    ...
    require(args.initialized.is_file(), f"Missing clean CBR initialization: {args.initialized}")
    ...
    require(audit["runtime"]["torch_version"].split("+")[0] == "2.1.2", "Formal C20 launch requires actual PyTorch 2.1.2 audit.")
    ...
    require(checkpoint.get("c20_provenance", {}).get("source_sha256") == SOURCE_SHA256,
    ...
    plan = {
        ...
        "initialized_sha256": checkpoint_hash, "model_yaml": str(C20_CFG),
...
def claim_launch(plan):
    ...
    lock = target.with_name(target.name + ".c20.launch.lock")
...
def launch_status(report_dir, name):
    ...
    lock = target.with_name(target.name + ".c20.launch.lock") if target else None
    return {"session": session, "tmux_active": active, "processes": processes, "records": records,
...
def assert_no_active_launch(plan):
    ...
    require(not state["tmux_active"] and not state["processes"], f"Existing C20 session/process: {state}")
...
def install_preflight(model, plan, report_dir):
    ...
    def before_first_batch(trainer):
        from audit_rtdetr_r18_lite_c20 import optimizer_rows, state_hashes
        actual_args_check(vars(trainer.args), plan["target_args"])
...
def main() -> None:
    ...
    parser.add_argument("--audit-report", type=Path, default=ROOT / "outputs/c20/audit.json")
    parser.add_argument("--name", default=DEFAULT_NAME)
    parser.add_argument("--report-dir", type=Path, default=ROOT / "outputs/c20/launch_c20")
    ...
    if args.tmux or args.execute:
        marker = ROOT / "outputs/c20/prepare.state"
        require(marker.is_file() and marker.read_text().strip() == "passed",
                "Latest prepare did not complete successfully; run bash tools/autodl_c20.sh prepare and inspect its error logs.")
    plan, _ = prepare(args)
    if args.tmux:
        require(os.name == "posix", "tmux launch is intended for the Linux server.")
        token = claim_launch(plan)
        session = f"c20_{args.name}"
        ...
        supervisor = [sys.executable, "-u", str(ROOT / "tools/c20_tmux_worker.py"),
                      str(args.report_dir.resolve()), *command]
        # An existing C15 tmux server may have a different environment; pin CBR import path explicitly.
        shell_command = shlex.join(["env", f"PYTHONPATH={ROOT / 'ultralytics-main'}", *supervisor])
        shell_command += "; rc=$?; printf '\\nC20 finished: exit=%s. Logs retained; press Enter to open shell.\\n' \"$rc\"; read -r _; exec bash -i"
        try:
