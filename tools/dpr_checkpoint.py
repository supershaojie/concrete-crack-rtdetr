"""DPR-only native checkpoint policy: keep optimizer moments in FP32.
...
def native_serializer_identity():
    ...
    if (identity["native_serializer_sha256"] != NATIVE_SERIALIZER_SOURCE_SHA256 and
        ...
        raise RuntimeError("Native save_model semantics changed; review DPR checkpoint override before saving")
    return identity
...
def require_checkpoint_policy(checkpoint):
    ...
    """
    metadata = checkpoint.get("DPR_checkpoint", {})
    ...
    required = {"epoch", "best_fitness", "model", "ema", "updates", "optimizer", "scaler", "train_args"}
    if not required <= set(checkpoint):
        raise RuntimeError("Checkpoint omits native continuation fields: " + str(sorted(required - set(checkpoint))))
    if type(checkpoint["epoch"]) is not int or checkpoint["epoch"] < 0:
        raise RuntimeError("Initialization or completed stripped checkpoint cannot be resumed")
    if type(checkpoint["updates"]) is not int or checkpoint["updates"] < 1:
        raise RuntimeError("Checkpoint has no valid EMA update count")
    if not isinstance(checkpoint["train_args"], dict) or checkpoint["train_args"].get("optimizer") != "AdamW":
        raise RuntimeError("Checkpoint does not contain the original AdamW training arguments")
    scaler = checkpoint["scaler"]
    if not isinstance(scaler, dict):
        raise RuntimeError("Checkpoint scaler field is missing or malformed")
    if scaler and not {"scale", "growth_factor", "backoff_factor", "growth_interval", "_growth_tracker"} <= set(scaler):
        raise RuntimeError("Native AMP scaler state is incomplete")
    if scaler and (not all(torch.isfinite(torch.tensor(float(value))) for value in scaler.values()) or scaler["scale"] <= 0):
        raise RuntimeError("Native AMP scaler state is nonfinite or invalid")
    if type(metadata.get("scaler_enabled")) is not bool or bool(scaler) != metadata["scaler_enabled"]:
        raise RuntimeError("Saved scaler enabled mode disagrees with original scaler state")
    if metadata.get("purpose") not in {"formal_training", "lifecycle_diagnostic", "bounded_capacity"}:
        raise RuntimeError("Checkpoint purpose is absent or unsupported")
    ...
    if not isinstance(ema, torch.nn.Module):
        raise RuntimeError("Native DPR resume requires its original EMA model")
    ...
    for parameter_id, state in optimizer_state["state"].items():
        ...
        step = state["step"]
        if not isinstance(step, torch.Tensor) or step.numel() != 1 or not bool(torch.isfinite(step)) or float(step) <= 0:
            raise RuntimeError("Invalid AdamW step counter")
...
class DPRCheckpointTrainer(RTDETRTrainer):
        ...
        for state in optimizer_state["state"].values():
            for name in ("exp_avg", "exp_avg_sq", "max_exp_avg_sq"):
                ...
                if value is not None and (not isinstance(value, torch.Tensor) or value.dtype != torch.float32):
                    raise RuntimeError(
                        "DPR requires active FP32 AdamW moments; old FP16 checkpoint information cannot be recovered by casting"
                    )
        metadata = dict(policy=OPTIMIZER_POLICY, optimizer_param_names=names,
                        scaler_enabled=self.scaler.is_enabled(),
                        purpose=getattr(self, "dpr_checkpoint_purpose", "formal_training"), **identity)
        # The inherited file-only Git reader does not resolve refs through a
        # linked worktree's commondir. Keep its native fields and record the
        # actual full HEAD separately for reliable formal resume identity.
        metadata["code_head"] = subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=Path(__file__).resolve().parents[1], text=True
        ).strip()
        if len(metadata["code_head"]) != 40 or any(c not in "0123456789abcdef" for c in metadata["code_head"]):
            raise RuntimeError("Cannot resolve full worktree HEAD for DPR checkpoint identity")
