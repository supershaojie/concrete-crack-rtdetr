"""Audit C21 initialization, decoder behavior, dtypes and optional real-data smoke. No formal training."""
...
from init_rtdetr_r18_lite_sala_controlled import (
    BASE_CFG, DEFAULT_SOURCE, DEFAULT_OUTPUT, ROOT, SOURCE_SHA256, SALA_CFG,
...
def compare_output(a, b, path="output"):
    if isinstance(a, torch.Tensor):
        ...
        require(torch.equal(a, b), f"{path}: bitwise equivalence failed; max_abs={(a.float()-b.float()).abs().max().item()}")
...
def audit_initialization(source, initialized, recipe_path=None):
    ...
    require(clean_checkpoint(checkpoint), "SALA contains training state.")
    ...
    for nc in (1, 80):
        ...
        torch.manual_seed(42)
        fresh_target = RTDETRDetectionModel(str(SALA_CFG), nc=nc, verbose=False)
...
def audit_real_train_api(initialized, nc, expected, expected_rng, recipe_path=None):
    ...
    from train_rtdetr_r18_lite_c21 import actual_args_check, build_locked_args, DEFAULT_NAME
...
def audit_wiring(initialized):
    ...
    for name, module in branch_modules(model).items():
        def hook(m, inputs, output, name=name):
            captured[name] = {'class': type(m).__name__, 'query': list(inputs[0].shape),
                'reference_boxes': list(inputs[1].shape), 'value': list(inputs[2].shape),
                'value_shapes': inputs[3], 'output': list(output.shape),
                'parameters': sum(p.numel() for p in m.parameters()),
                'added_parameters': sum(p.numel() for n,p in m.named_parameters() if n.startswith('sala_'))}
        handles.append(module.register_forward_hook(hook))
    try:
        with torch.inference_mode(): model(torch.rand(1,3,640,640))
        require(len(captured) == 3, 'Not all three SALA modules executed.')
        for row in captured.values():
            require(row['query'] == row['output'] == [1,300,256], 'Attention shape changed.')
            require(row['value_shapes'] == [[80,80],[40,40],[20,20]], 'Pyramid order differs.')
        return {'cross_attention': captured, 'distinct_module_objects': len({id(m) for m in branch_modules(model).values()}),
                'all_parameters_independent': True, 'decoder_inputs': model.model[-1].f,
                'complete_attention_parameters': sum(r['parameters'] for r in captured.values()),
                'added_parameters': sum(r['added_parameters'] for r in captured.values())}
    finally:
...
def benchmark(initialized, source, device):
    ...
    for label, w in (("C2", base_weights), ("C21", weights)):
...
def run_regressions(report_dir):
    result = {}
    for pattern in ("test_sala.py", "test_c21_tools.py"):
...
def main():
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "outputs/c21/audit.json")
    ...
    parser.add_argument("--smoke-dir", type=Path, default=ROOT / "outputs/c21/smoke")
    ...
    try:
        ...
        if args.smoke_data:
            from smoke_rtdetr_r18_lite_c21 import smoke
