...
import torch
from init_lif_down import ROOT, MODEL_DIR, VARIANTS, initialize, require, runtime, sha256, verify_model, write_json, is_added, BASE_COMMIT
...
MAIN = Path(os.environ.get("LIF_DOWN_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))
...
def record_source(folder, c2_args, data):
    ...
    subprocess.run(["git", "archive", "--format=tar.gz", "--output=" + str(folder / "source_snapshot.tar.gz"), "HEAD",
                    ...
                    "configs", "docs/lif_down", "docs/lif_down/c2_args.yaml", ".gitattributes"], cwd=ROOT, check=True)
...
def paths(variant):
    ...
    return dict(name=name, run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/lif_down",
                init=ROOT / "weights" / "lif_down_controlled_init.pt",
...
def recipe(c2_path, variant, init):
    source, expected = YAML.load(c2_path), YAML.load(ROOT / "docs/lif_down/c2_args.yaml")
    ...
    for row in rows:
        if row["changed"]:
            row["reason"] = "experiment model/output identity" if row["field"] in {"model", "name", "save_dir"} else "explicit LIF_DOWN_MAIN root relocation; same data/recipe required"
    return target, rows


def verify_data_config(path):
    source, expected = YAML.load(path), YAML.load(ROOT / "docs/lif_down/c2_data.yaml")
...
def build_training_model(cfg, weights, data, variant):
    """Use the native trainer's current RNG for BOTH nc adaptations, without advancing it twice."""
    with torch.random.fork_rng(devices=[]):
        baseline = RTDETRDetectionModel(str(MODEL_DIR / VARIANTS[variant][1]), nc=data["nc"], ch=data["channels"], verbose=False)
        baseline.load(weights, verbose=False)
    model = RTDETRDetectionModel(cfg, nc=data["nc"], ch=data["channels"], verbose=False)
    model.load(weights, verbose=False)
    audit = rebuild_audit(weights, model, variant)
    after = model.state_dict()
    public = baseline.state_dict()
    require(all(k in after and torch.equal(v, after[k]) for k,v in public.items()), "nc1 initialization differs from native C2 adaptation")
    audit.update(c2_nc1_public_states=len(public), c2_nc1_public_exact=True,
                 added_trainable_parameters=sum(p.numel() for n,p in model.named_parameters() if is_added(n)))
    return model, audit
...
def rebuild_audit(source, target, variant):
    ...
    return dict(nc_source=source.model[-1].nc, nc_target=target.model[-1].nc, loaded_exact=len(equal),
                ...
                COMMON=equal, NEW=[k for k in equal if is_added(k)], MISSING=[], UNEXPECTED=[],
                SHAPE_MISMATCH=[], ALLOWED_CLASS_ADAPTATION=sorted(incompatible))
...
def verify_delivery():
    ...
    record = ROOT / "outputs/lif_down_delivery.json"
    require(record.is_file(), "Run sync_lif_down.sh with the delivered full SHA first")
    info = json.loads(record.read_text(encoding="utf-8"))
    head = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
    require(info["commit"] == head and info["branch"] == "codex/lif-down", "Pinned delivery changed")
...
def duplicate_processes():
    ...
    for item in Path("/proc").glob("[0-9]*/cmdline"):
        try:
            ...
            if int(item.parent.name) != os.getpid() and any(v.endswith(b"train_lif_down.py") for v in argv) and b"worker" in argv:
...
def start_direct(variant):
    ...
    require(not duplicate_processes(), "Existing LIF-Down worker process protected")
    require(not p["launch"].exists() and not p["init"].exists(), "Existing LIF-Down launch/init protected")
    require(ROOT.resolve() != MAIN.resolve(), "Use the independent LIF-Down worktree, not the main checkout")
    ...
    lock = p["run"].with_name(p["run"].name + ".lif_down.lock")
    ...
    try:
        ...
        command = [sys.executable, "-u", str(ROOT / "tools/train_lif_down.py"), "worker", variant]
        plan["command"] = command
        write_json(p["launch"] / "plan.json", plan)
        # An existing tmux server may have another experiment's environment.
        env = {"PYTHONPATH": str(ROOT / "ultralytics-main"), "YOLO_AUTOINSTALL": "false", "LIF_DOWN_MAIN": str(MAIN), "CONDA_DEFAULT_ENV": os.environ.get("CONDA_DEFAULT_ENV")}
...
def run_state(p):
    ...
    if launch.get("status") == "dispatched" and shutil.which("tmux"):
        if subprocess.run(["tmux", "has-session", "-t", "lif-down-training"], capture_output=True).returncode:
            return "FAILED"
...
def status(variant):
    ...
    if shutil.which("tmux"):
        active = subprocess.run(["tmux", "has-session", "-t", "lif-down-training"], capture_output=True).returncode == 0
