...
set -Eeuo pipefail
MODE="${1:-status}"
case "$MODE" in start-direct|preflight-only|status|val|test|pack-complete) ;; *) echo 'Expected start-direct/preflight-only/status/val/test/pack-complete'; exit 2 ;; esac
C19_LIF_V1_ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
[[ $# -eq 1 ]] || { echo 'Usage: bash tools/autodl_c19_lif_v1.sh start-direct|preflight-only|status|val|test|pack-complete'; exit 2; }
...
case "$MODE" in
    ...
    start-direct|preflight-only|val|test|pack-complete)
        ...
        elif [[ "$MODE" == preflight-only ]]; then
            "$C19_LIF_V1_PYTHON" -u tools/preflight_c19_lif_v1.py 2>&1 | tee "$C19_LIF_V1_LOG"
            C19_LIF_V1_CODES=("${PIPESTATUS[@]}")
        else
