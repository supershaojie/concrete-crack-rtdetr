...
import torch
from init_scca import ROOT, VARIANTS, initialize, require, runtime, sha256, verify_model, write_json
from ultralytics import RTDETR
from ultralytics.models.rtdetr.train import RTDETRTrainer
from ultralytics.utils import YAML, ASSETS
from ultralytics.data.utils import check_det_dataset

MAIN = Path(os.environ.get("SCCA_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))


def paths(variant):
    name = VARIANTS[variant][2]
    return dict(name=name, run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/scca" / variant,
                init=ROOT / "weights" / f"{variant}_scca_controlled_init.pt",
...
def start_direct(variant):
    ...
    lock = p["run"].with_name(p["run"].name + ".scca.lock")
    ...
    try:
        ...
        command = [sys.executable, "-u", str(ROOT / "tools/train_scca.py"), "worker", variant]
        # An existing tmux server may have another experiment's environment.
        env = {"PYTHONPATH": str(ROOT / "ultralytics-main"), "YOLO_AUTOINSTALL": "false", "SCCA_MAIN": str(MAIN)}
...
def status(variant):
    ...
    if shutil.which("tmux"):
        active = subprocess.run(["tmux", "has-session", "-t", "scca-" + variant], capture_output=True).returncode == 0
