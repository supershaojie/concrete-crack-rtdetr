"""Prepare a field-locked C2 -> C18 launch plan. Training requires --execute or --tmux explicitly."""
...
from init_rtdetr_r18_lite_gsdr_aifi_v3_controlled import (
    DEFAULT_OUTPUT, ROOT, SOURCE_SHA256, V3_CFG, clean_checkpoint, require, runtime_info, sha256,
...
DEFAULT_NAME = "c18_rtdetr_r18_lite_gsdr_aifi_v3_e200_b16_onlineaug"
...
def build_locked_args(baseline: dict, initialized: Path, name: str) -> tuple[dict, list[dict]]:
    ...
    rows = [{"field": key, "C2": baseline[key], "V3": target[key], "equal": typed_equal(baseline[key], target[key]),
             "C2_type": type(baseline[key]).__name__, "V3_type": type(target[key]).__name__,
...
def prepare(args: argparse.Namespace) -> tuple[dict, Path]:
    ...
    require(args.initialized.is_file(), f"Missing clean V3 initialization: {args.initialized}")
    ...
    require(audit["runtime"]["torch_version"].split("+")[0] == "2.1.2", "Formal C18 launch requires actual PyTorch 2.1.2 audit.")
    ...
    plan = {
        ...
        "initialized_sha256": checkpoint_hash, "model_yaml": str(V3_CFG),
...
def claim_launch(plan):
    ...
    lock = target.with_name(target.name + ".c18.launch.lock")
...
def install_preflight(model, plan, report_dir):
    ...
    def before_first_batch(trainer):
        from audit_rtdetr_r18_lite_gsdr_aifi_v3 import optimizer_rows, state_hashes
        actual_args_check(vars(trainer.args), plan["target_args"])
...
def main() -> None:
    ...
    parser.add_argument("--audit-report", type=Path, default=ROOT / "outputs/gsdr_aifi_v3/audit.json")
    parser.add_argument("--name", default=DEFAULT_NAME)
    parser.add_argument("--report-dir", type=Path, default=ROOT / "outputs/gsdr_aifi_v3/launch_c18")
    ...
    if args.tmux:
        ...
        session = f"gsdr_aifi_v3_{args.name}"
        ...
        supervisor = [sys.executable, "-u", str(ROOT / "tools/gsdr_aifi_v3_tmux_worker.py"),
                      str(args.report_dir.resolve()), *command]
        # An existing C15 tmux server may have a different environment; pin V3 import path explicitly.
