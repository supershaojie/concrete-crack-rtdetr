"""RSC-Head native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_rsc_head import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_rsc_head import MAIN, paths, run_state
...
def validate_complete(p):
    ...
    require(all((launch / n).is_file() for n in required), "RSC-Head direct-start metadata incomplete")
...
def missing_evidence(p):
    missing = []
    for name in ("checks.json", "initialization_mapping.json", "ops_checks.json", "validation.md"):
        if not (ROOT / "docs/rsc_head" / name).is_file(): missing.append("metadata/source/docs/rsc_head/" + name)
    required_launch = ("initialization.json", "nc1_loading.json", "training_setup.json", "actual_train_args.yaml",
                       ...
                       "process_exit_code.txt", "amp_resources.json")
...
def package(destination=None):
    """Archive all available evidence; explicitly mark incomplete packs, never run inference."""
    p = paths("rsc_head")
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/rsc_head" / f"rsc_head_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    ...
    tree(run, "training")
    if (ROOT / "outputs/rsc_head_delivery.json").is_file():
        files["metadata/rsc_head_delivery.json"] = ROOT / "outputs/rsc_head_delivery.json"
    tree(launch, "metadata/launch", ("evaluation_val", "evaluation_test"))
    for split in ("val", "test"):
        tree(launch / ("evaluation_" + split), "evaluation/" + split)
    logs = ROOT / "outputs/rsc_head_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/rsc_head/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/rsc_head/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed RSC-Head directories")
        p = paths("rsc_head")
