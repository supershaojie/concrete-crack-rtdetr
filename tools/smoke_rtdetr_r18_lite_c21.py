...
from init_rtdetr_r18_lite_sala_controlled import require, sha256, write_json, verify_module, branch_modules
...
def smoke(initialized, data, c2_args, output, expected_states):
    from audit_rtdetr_r18_lite_c21 import state_hashes, branch_gradients, optimizer_rows, compare_output
    from train_rtdetr_r18_lite_c21 import build_locked_args, DEFAULT_NAME
    ...
    report = {"status": "failed", "data": str(Path(data).resolve()), "data_sha256": sha256(data),
              "amp_validation": "Explicit SALA AMP forward/backward and three optimizer updates; inspect console for native YOLO helper status.",
    ...
    class SmokeTrainer(RTDETRTrainer):
        def _do_train(self):
            ...
            image = None
            for module in branch_modules(self.model).values(): module.sala_stats_interval = 1
            for step in range(3):
                ...
                report["steps"].append({"step": step, "loss": float(loss.detach()), "gt_count": count,
                                        ...
                                        "allocation": {n:m.sala_last_stats for n,m in branch_modules(self.model).items()}})
            require(all(v > 0 for v in grads.values()), "Real updates did not open every SALA parameter tensor.")
            first = report['steps'][0]['module_gradients']
            require(all(v > 0 for k,v in first.items() if k.endswith('sala_level_head.weight')), 'Zero head weight received no gradient.')
            require(all(v == 0 for k,v in first.items() if '.sala_level_head.' not in k), 'Upstream first-step gradient should be zero.')
            for module in branch_modules(self.model).values():
                module.sala_stats_interval = 0; module.sala_last_stats = None
            ...
            require(torch.isfinite(half_out).all(), 'Trained SALA half inference failed.')
