...
from train_mincompat_v2 import recipe
...
def run(pipeline,checkpoint,output):
    ...
    c2=ROOT/'docs/mincompat_v2/c2_args.yaml';shutil.copyfile(c2,launch/'authoritative_c2_args.yaml')
    shutil.copyfile(pipeline/'subset/data.yaml',launch/'data_config.yaml')
    args,diff=recipe(c2,'triad_mincompat_v2',checkpoint)
    for name in ('train_args.yaml','actual_train_args.yaml'):YAML.save(launch/name,args)
    YAML.save(training/'args.yaml',args)
    plan=dict(runtime=info,preflight='passed',args=args,c2_args_sha256=sha256(c2),init_sha256=sha256(checkpoint),variant='triad_mincompat_v2',token=token,fixture=True)
    ...
    with patch.object(results,'paths',return_value=p),patch.object(results,'evaluate',side_effect=AssertionError('Pack must never run eval')):
        archive=results.package('triad_mincompat_v2',output/'fixture_complete.tar.gz')
        inventory=results.verify_archive(archive)
        names={item['path'] for item in inventory}
        required={'metadata/source/tools/mincompat_v2.py',
                  'metadata/source/tools/train_mincompat_v2.py',
                  'metadata/source/ultralytics-main/ultralytics/nn/modules/cscef_v52_compat.py',
                  'metadata/source/docs/mincompat_v2/gradient_edge.json'}
        required.update('metadata/source/ultralytics-main/ultralytics/cfg/models/rt-detr/'+v['yaml'] for v in VARIANTS.values())
        require(required <= names, 'V2 source/YAML/gradient evidence missing from package; stage new files before fixture')
        try:results.package('triad_mincompat_v2',archive)
        except RuntimeError:pass
        else:raise AssertionError('Overwrote fixture archive')
        (launch/'evaluation_test/metrics.json').write_text('{}')
        try:results.package('triad_mincompat_v2',output/'invalid.tar.gz')
        except RuntimeError:pass
    ...
    write_json(output/'pack_validation.json',dict(status='passed',scope='explicit synthetic train-lifecycle fixture + real two-image val/test outputs; NOT formal training',
        ...
        no_implicit_evaluation=True,overwrite_rejected=True,broken_test_rejected=True,v2_source_yaml_gradient_evidence=True))
