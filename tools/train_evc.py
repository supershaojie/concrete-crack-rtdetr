...
import torch
from init_evc import ROOT, VARIANTS, initialize, require, runtime, sha256, verify_model, write_json, is_added, BASE_COMMIT
from ultralytics import RTDETR
from ultralytics.models.rtdetr.train import RTDETRTrainer
from ultralytics.utils import YAML, ASSETS
from ultralytics.data.utils import check_det_dataset

MAIN = Path(os.environ.get("EVC_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))
...
def record_source(folder, c2_args, data):
    ...
    subprocess.run(["git", "archive", "--format=tar.gz", "--output=" + str(folder / "source_snapshot.tar.gz"), "HEAD",
                    ...
                    "configs", "docs/evc", "docs/evc/c2_args.yaml", ".gitattributes"], cwd=ROOT, check=True)
...
def paths(variant):
    ...
    return dict(name=name, run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/evc",
                init=ROOT / "weights" / "evc_deform_controlled_init.pt",
...
def recipe(c2_path, variant, init):
    source, expected = YAML.load(c2_path), YAML.load(ROOT / "docs/evc/c2_args.yaml")
    ...
    for row in rows:
        if row["changed"]:
            row["reason"] = "experiment model/output identity" if row["field"] in {"model", "name", "save_dir"} else "explicit EVC_MAIN root relocation; same data/recipe required"
    return target, rows
...
def start_direct(variant):
    p = paths(variant)
    require(ROOT.resolve() != MAIN.resolve(), "Use the independent EVC worktree, not the main checkout")
    ...
    lock = p["run"].with_name(p["run"].name + ".evc.lock")
    ...
    try:
        ...
        command = [sys.executable, "-u", str(ROOT / "tools/train_evc.py"), "worker", variant]
        # An existing tmux server may have another experiment's environment.
        env = {"PYTHONPATH": str(ROOT / "ultralytics-main"), "YOLO_AUTOINSTALL": "false", "EVC_MAIN": str(MAIN), "CONDA_DEFAULT_ENV": os.environ.get("CONDA_DEFAULT_ENV")}
...
def run_state(p):
    ...
    if process:
        ...
        return "RUNNING" if token and token == process.get("process_token") else ("FINISHING" if code == 0 else "FAILED_INTERRUPTED")
...
def status(variant):
    ...
    if shutil.which("tmux"):
        active = subprocess.run(["tmux", "has-session", "-t", "evc-training"], capture_output=True).returncode == 0
