"""Prepare a field-locked C2 -> C19 launch plan. Training requires --execute or --tmux explicitly."""
...
from init_rtdetr_r18_lite_cbr_controlled import (
    DEFAULT_OUTPUT, ROOT, SOURCE_SHA256, CBR_CFG, clean_checkpoint, require, runtime_info, sha256,
...
DEFAULT_NAME = "c19_rtdetr_r18_lite_cbr_e200_b16_onlineaug"
...
def build_locked_args(baseline: dict, initialized: Path, name: str) -> tuple[dict, list[dict]]:
    ...
    rows = [{"field": key, "C2": baseline[key], "CBR": target[key], "equal": typed_equal(baseline[key], target[key]),
             "C2_type": type(baseline[key]).__name__, "CBR_type": type(target[key]).__name__,
...
def prepare(args: argparse.Namespace) -> tuple[dict, Path]:
    ...
    require(audit["runtime"]["torch_version"].split("+")[0] == "2.1.2", "Formal C19 launch requires actual PyTorch 2.1.2 audit.")
    ...
    plan = {
        ...
        "initialized_sha256": checkpoint_hash, "model_yaml": str(CBR_CFG),
...
def claim_launch(plan):
    ...
    lock = target.with_name(target.name + ".c19.launch.lock")
...
def launch_status(report_dir, name):
    ...
    lock = target.with_name(target.name + ".c19.launch.lock") if target else None
...
def assert_no_active_launch(plan):
    ...
    require(not state["tmux_active"] and not state["processes"], f"Existing C19 session/process: {state}")
...
def install_preflight(model, plan, report_dir):
    ...
    def before_first_batch(trainer):
        from audit_rtdetr_r18_lite_cbr import optimizer_rows, state_hashes
        actual_args_check(vars(trainer.args), plan["target_args"])
        ...
        write_json(report_dir / "preflight.json", {"status": "passed", "all_states_exact": len(actual),
                   "optimizer_parameters_exact": len(groups), "optimizer_empty": True, "ema_updates": 0,
...
def main() -> None:
    ...
    parser.add_argument("--audit-report", type=Path, default=ROOT / "outputs/cbr/audit.json")
    parser.add_argument("--name", default=DEFAULT_NAME)
    parser.add_argument("--report-dir", type=Path, default=ROOT / "outputs/cbr/launch_c19")
    ...
    if args.tmux:
        ...
        supervisor = [sys.executable, "-u", str(ROOT / "tools/cbr_tmux_worker.py"),
