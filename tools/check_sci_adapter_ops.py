...
import unittest
import torch
from sci_adapter import ROOT,BASE_COMMIT,require,write_json
...
case "$*" in
 ...
 'fetch origin codex/rtdetr-sci-adapter'|'cat-file -e '*) : ;;
...
'''
        def invoke(**extra):
            return subprocess.run([bash,'-c',command,'fixture',root.as_posix(),(ROOT/'tools/sync_sci_adapter.sh').as_posix(),BASE_COMMIT],
        ...
        pin=root/'worktree/outputs/sci_adapter_sync.json';pin.write_text('user file')
...
def run(bash,output):
    ...
    for name in ('test_sci_adapter_lifecycle.py','test_sci_adapter.py'):
    ...
    for name in ('autodl_sci_adapter.sh','sync_sci_adapter.sh'):
