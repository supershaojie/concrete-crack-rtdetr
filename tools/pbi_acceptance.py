"""Pure evidence validation for PBI restoration and independent trajectories.
...
PBI_SHAPES = {
    "model.17.pbi.W1.weight": [32, 256, 1, 1],
    "model.17.pbi.W2.weight": [32, 256, 1, 1],
    "model.17.pbi.Wo.weight": [256, 32, 1, 1],
}
FORWARD_KEYS = {
...
def _forward_keys(parameters):
    ...
    if set(PBI_SHAPES) <= set(parameters):
        keys.update({"pbi_input", "pbi_output", "pbi_residual", "pbi_u", "pbi_v"})
...
def _state_inventory(e, rows, names, path):
    ...
    nonpersistent = set()  # PBI has no buffers, persistent or otherwise.
    ...
    if names <= set(model):
        ...
        expected_total = (20149765 if has_cbr else 20082772) + (24576 if set(PBI_SHAPES) <= names else 0)
        e.need(total == expected_total, path, "parameter inventory does not match the fixed nc=1 architecture")
    for name, shape in PBI_SHAPES.items():
        if name in names:
            e.need(model.get(name, {}).get("shape") == shape, path + "." + name, "PBI projection shape differs")
...
def _source_names(e, report):
    ...
    e.need(len(set(names)) == len(names) and set(PBI_SHAPES) <= set(names), "optimizer_param_names", "duplicate/incomplete parameter names")
...
def evaluate_checkpoint(report, label, check_contract_version=False):
    ...
    try:
        ...
        _storage(e, restoration.get("live_direct_independence", {}), "restoration.live_direct_storage")
        _storage(e, restoration.get("live_resumed_independence", {}), "restoration.live_resumed_storage")
...
def _case(e, report, label, name, parameters, reference=None):
    ...
    for item in (first, second):
        ...
        norms = item.get("pbi_grad_norms", {})
        required_pbi = set(parameters) & set(PBI_SHAPES)
        e.need(set(norms) == required_pbi and all(_number(v, 1e-300) for v in norms.values()), path, "PBI gradient missing/nonfinite/not activated")
    ...
    for parameter in parameters & set(PBI_SHAPES):
        for prefix in ("model", "ema"):
            e.need(rows.get("state." + prefix + "." + parameter, {}).get("allclose") is True,
                   path + "." + parameter, "PBI parameter/EMA update exceeds the original tolerance")
    ...
    pbi_gradient_rows = {key: deepcopy(row) for key, row in gradients.items() if key[len("state."):] in PBI_SHAPES}
    return dict(rows=rows, gradients=gradients, initial=initial_rows,
                summary=dict(raw_status=report.get("raw_status", report.get("status")),
                             ...
                             pbi_gradient_comparison=pbi_gradient_rows,
                             forward_hashes_exact=report.get("captured_forward_hashes_exact"), loss_exact=report.get("loss_exact")))


def evaluate_mode(label, checkpoint, parent_live, pbi_live, check_contract_version=False):
    ...
    fitted to a particular package; PBI updates retain the original tolerance.
    """
    ...
    try:
        ...
        parent_names = [name for name in names if name not in PBI_SHAPES]
        cases["parent_live"] = _case(e, parent_live, label, "parent_live", parent_names)
        cases["pbi_live"] = _case(e, pbi_live, label, "pbi_live", names)
        cases["checkpoint"] = _case(e, checkpoint, label, "checkpoint", names, cases["pbi_live"]["initial"])
        if check_contract_version:
            for name, report in (("parent_live", parent_live), ("pbi_live", pbi_live)):
        ...
        if label != "cpu_fp32":
            ...
            if any_difference:
                e.need(changed["parent_live"] and changed["pbi_live"], "trajectory.mechanism", "both same-mode live controls must reproduce finite backward variation")
                for case_name, case in cases.items():
                    for key, row in case["rows"].items():
                        ...
                        if key.startswith(("state.model.", "state.ema.")):
                            ...
                            continue
                        if parameter in PBI_SHAPES:
                            # Gradient differences are retained and disclosed;
                            # PBI state updates were already checked above.
                            e.need(False, "trajectory." + case_name + "." + key, "unexpected PBI optimizer/state divergence")
                            continue
                        gradient_key = "state." + parameter
                        e.need(all(control["gradients"].get(gradient_key, {}).get("equal") is False
                                   for control in (case, cases["parent_live"], cases["pbi_live"])),
    ...
    return dict(contract_version=CONTRACT_VERSION, status="FAILED" if failures else trajectory_status,
                ...
                trajectory=dict(status=trajectory_status, failures=e.failures,
                    ...
                    note="A precision note is not raw trajectory equality or a detector accuracy claim; PBI gradient exceptions remain listed"),
                failures=failures)
