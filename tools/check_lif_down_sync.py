...
from init_lif_down import ROOT, require, write_json
import train_lif_down as train
...
def main():
    ...
    git('clone','--bare','--shared','--single-branch','--branch','codex/lif-down',str(ROOT),str(remote))
    ...
    git('push','origin','HEAD:refs/heads/codex/lif-down',cwd=main_repo)
...
if [[ "${3:-}" == fetch && "${4:-}" == origin ]]; then
 exec "$LIF_TEST_GIT" "$1" "$2" fetch "$LIF_TEST_REMOTE" "${@:5}"
fi
exec "$LIF_TEST_GIT" "$@"
''')
    ...
    env=dict(os.environ, LIF_TEST_GIT=realgit, LIF_TEST_REMOTE=str(remote),LIF_TEST_WINDOWS=str(int(os.name=='nt')),
             LIF_DOWN_PYTHON=sys.executable,PATH=str(wrapper)+os.pathsep+os.environ['PATH'])
    checks=[]
    def sync(name, sha=pin, target=worktree, success=True):
        ...
        cp=subprocess.run([args.bash,'-c',shell,'lif-sync-test',str(wrapper),str(ROOT/'tools/sync_lif_down.sh'),sha,str(main_repo),str(target)],env=env,capture_output=True,text=True)
