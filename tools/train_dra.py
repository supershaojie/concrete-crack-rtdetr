...
import torch
from init_dra import ROOT, VARIANTS, initialize, require, runtime, sha256, verify_model, write_json, is_added, BASE_COMMIT
from ultralytics import RTDETR
from ultralytics.models.rtdetr.train import RTDETRTrainer
from ultralytics.utils import YAML, ASSETS
from ultralytics.data.utils import check_det_dataset

MAIN = Path(os.environ.get("DRA_MAIN", "/root/autodl-tmp/projects/Crack_RTDETR"))


def verify_server_environment(info):
    require(torch.cuda.is_available(), "CUDA required; runtime is recorded without upgrading the environment")
...
def record_source(folder, c2_args, data):
    ...
    subprocess.run(["git", "archive", "--format=tar.gz", "--output=" + str(folder / "source_snapshot.tar.gz"), "HEAD",
                    ...
                    "configs", "docs/dra", ".gitattributes"], cwd=ROOT, check=True)
...
def paths(variant):
    ...
    return dict(name=name, run=MAIN / "runs/c_series" / name, launch=ROOT / "outputs/dra",
                init=ROOT / "weights" / "dra_aifi_controlled_init.pt",
...
def recipe(c2_path, variant, init):
    source, expected = YAML.load(c2_path), YAML.load(ROOT / "docs/dra/c2_args.yaml")
    ...
    target = dict(source)
    original_main = Path("/root/autodl-tmp/projects/Crack_RTDETR")
    target.update(model=str(Path(init).resolve()), name=VARIANTS[variant][2],
                  project=(MAIN / "runs/c_series").as_posix(),
                  data=(MAIN / Path(source["data"]).relative_to(original_main)).as_posix(),
                  save_dir=(MAIN / "runs/c_series" / VARIANTS[variant][2]).as_posix())
    explanations = dict(model="DRA clean initialization", name="Independent experiment alias",
                        save_dir="Independent run directory", project="Explicit DRA_MAIN root relocation",
                        data="Same dataset config relocated under explicit DRA_MAIN")
    rows = [{"field": k, "C2": source[k], variant: target[k], "changed": source[k] != target[k],
             "reason": explanations.get(k) if source[k] != target[k] else None} for k in sorted(source)]
    require({r["field"] for r in rows if r["changed"]} <= set(explanations), "Unexpected recipe difference")
    return target, rows
...
def start_direct(variant):
    p = paths(variant)
    require(ROOT.resolve() != MAIN.resolve(), "Formal DRA requires its independent worktree")
    require(subprocess.check_output(["git", "branch", "--show-current"], cwd=ROOT, text=True).strip() == "codex/dra-aifi",
            "DRA must run from codex/dra-aifi, not another experiment branch")
    ...
    require(not p["launch"].exists() and not p["init"].exists(), "Existing DRA launch/init evidence protected")
    session = "dra-aifi-training"
    ...
    lock = p["run"].with_name(p["run"].name + ".dra.lock")
    ...
    try:
        ...
        plan = dict(variant=variant, runtime=info, args=args, c2_args_sha256=sha256(p["c2_args"]),
                    ...
                    mode="start-direct", command=[sys.executable, "-u", str(ROOT / "tools/train_dra.py"), "worker", variant], created=datetime.now(timezone.utc).isoformat())
        write_json(p["launch"] / "plan.json", plan)
        worker = p["launch"] / "worker.sh"
        console = p["launch"] / "console.log"
        command = [sys.executable, "-u", str(ROOT / "tools/train_dra.py"), "worker", variant]
        # An existing tmux server may have another experiment's environment.
        env = {"PYTHONPATH": str(ROOT / "ultralytics-main"), "YOLO_AUTOINSTALL": "false", "DRA_MAIN": str(MAIN)}
        ...
        write_json(p["launch"] / "launch_state.json", dict(status="dispatched", command=command, session=session, log=str(console), full_server_preflight="NOT_RUN"))
...
def worker(variant):
    ...
    try:
        ...
        source_record = json.loads((p["launch"] / "source_record.json").read_text())
        require(sha256(plan["args"]["data"]) == source_record["data_sha256"], "Data config changed after dispatch")
...
def status(variant):
    ...
    state = "not_submitted"
    if (p["launch"] / "launch_state.json").exists():
        state = json.loads((p["launch"] / "launch_state.json").read_text())["status"]
    exit_path = p["launch"] / "process_exit_code.txt"
    process = p["launch"] / "process.json"
    if exit_path.exists():
        state = "successfully_exited" if exit_path.read_text().strip() == "0" else "failed"
    elif (p["launch"] / "exit_code.json").exists():
        code = json.loads((p["launch"] / "exit_code.json").read_text())["exit_code"]
        state = "successfully_exited" if code == 0 else "failed"
    elif process.exists():
        pid = json.loads(process.read_text())["pid"]
        try:
            os.kill(pid, 0)
            cmdline = Path(f"/proc/{pid}/cmdline")
            require(cmdline.exists() and "train_dra.py" in cmdline.read_text(), "PID reused")
            state = "training" if (p["launch"] / "training_state.json").exists() else "worker_starting"
        except (OSError, RuntimeError):
            state = "failed_worker_missing"
    elif state == "dispatched" and shutil.which("tmux"):
        if subprocess.run(["tmux", "has-session", "-t", "dra-aifi-training"], capture_output=True).returncode:
            state = "failed_dispatch_lost"
    print("state:", state)
