...
from init_c17_lif_v1 import ROOT, require, runtime, sha256, write_json
import train_c17_lif_v1 as train
import c17_lif_v1_results as results
...
def run(output):
    ...
    with tempfile.TemporaryDirectory(prefix="c17_lif_v1_ops_", dir=ROOT / "outputs") as d:
        temporary = Path(d)
        main, root = temporary / "main", temporary / "worktree"
        root.mkdir(); (main / "configs").mkdir(parents=True)
        (root / "docs/c17_lif_v1").mkdir(parents=True)
        (root / "docs/c17_lif_v1/c2_args.yaml").write_bytes((ROOT / "docs/c17_lif_v1/c2_args.yaml").read_bytes())
        ...
        p = dict(name="fixture", run=main / "runs/c_series" / train.VARIANTS["c17_lif_v1"][2], launch=root / "outputs/c17_lif_v1", init=root / "init.pt", source=source,
                 c2_args=ROOT / "docs/c17_lif_v1/c2_args.yaml")
        calls = []
        ...
        with ExitStack() as stack:
            for obj, name, value in ((train, "ROOT", root), (train, "MAIN", main), (train, "paths", lambda _: p),
                                     ...
                                     (train, "bounded_preflight", lambda *a: None),
            ...
            train.start_direct("c17_lif_v1")
            plan = json.loads((p["launch"] / "plan.json").read_text(encoding="utf-8"))
            require(plan["args"]["batch"] == 16 and plan["args"]["epochs"] == 200 and plan["full_server_preflight"] == "PASSED", "Direct recipe changed")
            ...
            report["direct_with_mocked_finite_preflight"] = "passed (mocked dispatch)"
            report["duplicate_reservation_rejected"] = rejected(lambda: train.start_direct("c17_lif_v1"))
