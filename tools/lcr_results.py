"""LCR native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from init_lcr import ROOT, require, runtime, sha256, verify_model, write_json, SOURCE_SHA256
from train_lcr import MAIN, paths, run_state
...
def evaluate(weights, data, split, output, device="0", val_report=None, evidence_scope="full_split"):
    ...
    print(json.dumps(dict(runtime=info, evidence_scope=evidence_scope, split=split), ensure_ascii=False), flush=True)
    if split == "test":
...
def validate_complete(p):
    ...
    require(all((launch / n).is_file() for n in required), "LCR direct-start metadata incomplete")
...
def package(destination=None):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/lcr" / f"lcr_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    ...
    tree(run, "training")
    if (ROOT / "outputs/lcr_delivery.json").is_file():
        files["metadata/lcr_delivery.json"] = ROOT / "outputs/lcr_delivery.json"
    tree(launch, "metadata/launch", ("evaluation_val", "evaluation_test"))
    for split in ("val", "test"):
        tree(launch / ("evaluation_" + split), "evaluation/" + split)
    logs = ROOT / "outputs/lcr_command_logs"
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/lcr/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/lcr/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed LCR directories")
        p = paths("lcr_aifi")
