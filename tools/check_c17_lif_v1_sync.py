...
import train_c17_lif_v1 as train
...
def main():
    ...
    def git(*argv, cwd=ROOT):
        cp=subprocess.run([realgit,*argv],cwd=cwd,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
        require(cp.returncode==0,cp.stdout)
        return cp.stdout.strip()
    base = git('rev-parse','HEAD')
    # Snapshot current candidate files into a fixture commit, before committing the real delivery.
    candidate=folder/'candidate'
    bundle=folder/'source.bundle'
    git('bundle','create',str(bundle),'codex/rtdetr-c17-lif-v1')
    git('clone','--single-branch','--branch','codex/rtdetr-c17-lif-v1',str(bundle),str(candidate))
    names=git('ls-files','--cached','--others','--exclude-standard','-z').split('\0')
    for name in names:
        if name and (ROOT/name).is_file():
            dest=candidate/name;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(ROOT/name,dest)
    git('add','-A',cwd=candidate)
    git('-c','user.name=Fixture','-c','user.email=fixture@example.invalid','commit','-m','candidate source fixture',cwd=candidate)
    pin=git('rev-parse','HEAD',cwd=candidate)
    remote, main_repo, worktree = folder/'remote.git', folder/'main', folder/'worktree with space'
    git('clone','--bare','--shared','--single-branch','--branch','codex/rtdetr-c17-lif-v1',str(candidate),str(remote))
    ...
    git('push','origin','HEAD:refs/heads/codex/rtdetr-c17-lif-v1',cwd=main_repo)
...
if [[ "${3:-}" == fetch && "${5:-}" == origin ]]; then
 exec "$LIF_TEST_GIT" "$1" "$2" fetch --no-write-fetch-head "$LIF_TEST_REMOTE" "${@:6}"
fi
exec "$LIF_TEST_GIT" "$@"
''')
    ...
    env=dict(os.environ, LIF_TEST_GIT=realgit, LIF_TEST_REMOTE=str(remote),LIF_TEST_WINDOWS=str(int(os.name=='nt')),
             C17_LIF_V1_PYTHON=sys.executable,PATH=str(wrapper)+os.pathsep+os.environ['PATH'])
    if os.name=='nt':
        git_root=Path(args.bash).resolve().parents[1]
        env['PATH']=str(wrapper)+os.pathsep+str(git_root/'usr/bin')+os.pathsep+str(git_root/'mingw64/bin')+os.pathsep+os.environ['PATH']
    checks=[]
    def sync(name, sha=pin, target=worktree, success=True):
        shell = 'export PATH="/usr/bin:/mingw64/bin:$PATH"; test_bin="$1"; shift; if command -v cygpath >/dev/null; then test_bin="$(cygpath -u "$test_bin")"; fi; export PATH="$test_bin:$PATH"; source "$@"'
        cp=subprocess.run([args.bash,'-c',shell,'lif-sync-test',str(wrapper),str(ROOT/'tools/sync_c17_lif_v1.sh'),sha,str(main_repo),str(target)],env=env,capture_output=True,text=True)
    ...
    require(git('rev-parse','refs/remotes/origin/codex/rtdetr-c17-lif-v1',cwd=main_repo)==advance, 'Fixture remote redirection/advancement not exercised')
    ...
    dirty=worktree/'README.md';previous=dirty.read_bytes();dirty.write_bytes(previous+b'preserve fixture')
    sync('dirty_target_rejected',success=False)
    require(dirty.read_bytes()==previous+b'preserve fixture','Dirty target overwritten')
    # This file belongs only to this test fixture, never an experiment checkout.
    dirty.write_bytes(previous)
    extra=worktree/'untracked_result.txt';extra.write_bytes(b'preserve generated output')
    sync('untracked_output_preserved')
    require(extra.read_bytes()==b'preserve generated output','Untracked result lost')
    ...
    write_json(args.output,dict(status='PASSED',scope='Real local Git/worktrees; only fetch destination redirected to isolated local bare repository',
               base_commit=base,pinned_fixture_commit=pin,fixture_remote_advanced_commit=advance,detached_launcher_accepted=True,main_dirty_state_preserved=True,
