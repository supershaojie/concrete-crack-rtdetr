...
from init_triad_compat import initialize, rebuild_audit
from ultralytics import RTDETR
from ultralytics.models.rtdetr.train import RTDETRTrainer
from ultralytics.data.utils import check_det_dataset
from ultralytics.utils import ASSETS

MAIN=Path(os.environ.get('TRIAD_MAIN','/root/autodl-tmp/projects/Crack_RTDETR'))
...
def paths(variant):
    ...
    return dict(variant=variant,name=v['name'],run=MAIN/'runs/c_series'/v['name'],
        ...
        session=f'triad-{variant}',lock=MAIN/'runs/c_series'/f'{v["name"]}.triad.lock')


def recipe(c2_path,variant,init):
    source=YAML.load(c2_path);expected=YAML.load(ROOT/'docs/triad_compat/c2_args.yaml')
...
def verify_environment(info):
    ...
    sync=read_json(ROOT/'outputs/triad_compat_sync.json')
    require(sync.get('sha')==info['commit'],'Run fixed-SHA sync first; source pin missing/mismatched')
    git('merge-base','--is-ancestor',BASE_COMMIT,info['commit'])
...
def amp_resources():
    ...
    for dest,candidates in [(ASSETS/'bus.jpg',[MAIN/'ultralytics-main/ultralytics/assets/bus.jpg',MAIN/'bus.jpg']),
        ...
        if not dest.is_file():
            ...
            with tempfile.NamedTemporaryFile(dir=dest.parent,prefix='.triad_amp_',delete=False) as tmp:
...
def record_source(folder,p,args):
    ...
    subprocess.run(['git','archive','--format=tar.gz','--output='+str(folder/'source_snapshot.tar.gz'),'HEAD',
        'tools','ultralytics-main/ultralytics','ultralytics-main/tests','configs','docs/triad_compat','.gitattributes'],cwd=ROOT,check=True)
...
def start_direct(variant):
    ...
    require(ROOT.resolve()!=MAIN.resolve(),'Use the independent Triad worktree')
    ...
    for proc in Path('/proc').glob('[0-9]*/cmdline'):
        ...
        require(not (any(a.endswith('train_triad_compat.py') for a in argv) and 'worker' in argv and variant in argv),'Same variant worker exists')
    ...
    try:
        ...
        require(YAML.load(args['data'])==YAML.load(ROOT/'docs/triad_compat/c2_data.yaml'),'Dataset/split configuration differs from archived C2')
        ...
        from check_triad_compat import smoke, precision
        from check_triad_compat_gradients import run_checks
        target=RTDETR(str(p['init'])).model
        preflight=dict(status='passed',gradient_firewall=run_checks(),AMP_loss_DN=smoke(target,variant,'cuda',True),
        ...
        command=[sys.executable,'-u',str(ROOT/'tools/train_triad_compat.py'),'worker',variant,'--token',token]
        ...
        env.update(PYTHONPATH=str(ROOT/'ultralytics-main'),YOLO_AUTOINSTALL='false',TRIAD_MAIN=str(MAIN))
