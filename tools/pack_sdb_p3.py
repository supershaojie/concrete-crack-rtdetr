...
from init_sdb_p3 import ROOT, VARIANTS, require, sha256
from train_sdb_p3 import DEFAULT_VARIANT, paths, run_state, head, code_identity, write_json
...
def package(destination, variant=DEFAULT_VARIANT):
    ...
    for prefix, folder in (("training", p["run"]), ("metadata/launch", p["launch"]),
                           ("metadata/preflight", p["preflight"].parent), ("metadata/source/docs/sdb_p3", ROOT / "docs/sdb_p3")):
        ...
        for file in sorted(folder.rglob("*")):
            ...
            if file.suffix == ".log":
                with file.open("rb") as stream:
                    ...
                    ("results", "Box", "PR_", "P_", "R_", "F1_", "confusion_matrix", "labels", "train_batch", "val_batch"))):
                files[name] = file
            else:
