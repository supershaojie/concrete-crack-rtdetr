"""Create a clean FP32 CSCEF-v5 initialization from the hash-verified C2 checkpoint (no training)."""
...
V5_CFG = MODEL_DIR / "rtdetr-resnet18-lite-cscef-v5.yaml"
DEFAULT_SOURCE = ROOT / "weights/rtdetr_r18_lite_imagenet_backbone_init.pt"
DEFAULT_OUTPUT = ROOT / "weights/rtdetr_r18_lite_cscef_v5_imagenet_backbone_init.pt"
...
V4_COMMIT = "f34502c43a88edc9cb1076bd996b9c3fc2d23e31"
NEW_SUFFIXES = {
...
from ultralytics.nn.modules import CSCEFv5  # noqa: E402
...
def runtime_info() -> dict:
    ...
    audited_files = [
        ...
        "tools/init_rtdetr_r18_lite_cscef_v5_controlled.py", "tools/audit_rtdetr_r18_lite_cscef_v5.py",
    ]
    return {
...
def verify_module(model: torch.nn.Module) -> None:
    require(isinstance(model.model[18], CSCEFv5), "Expected CSCEFv5 at layer 18.")
    module = model.model[18]
    require(sum(p.numel() for p in module.parameters()) == 26912, "Unexpected V5 parameter count.")
    require(set(module.state_dict()) == NEW_SUFFIXES, "Unexpected V5 state keys.")
...
def verify_reloads(output: Path, expected: dict) -> dict:
    ...
    yaml_loaded = RTDETR(str(V5_CFG)).load(str(output)).model
    report = {}
...
def initialize(source: Path, output: Path) -> dict:
    ...
    report = {"runtime": runtime_info(), "source": str(source), "source_sha256": SOURCE_SHA256,
              "base_commit": BASE_COMMIT, "v4_commit": V4_COMMIT, "seed": 42,
    ...
    with torch.random.fork_rng(devices=[]):
        ...
        torch.random.default_generator.manual_seed(42)
        target = RTDETRDetectionModel(str(V5_CFG), ch=3, nc=80, verbose=False).eval()
    ...
    target.args = {**DEFAULT_CFG_DICT, "model": str(V5_CFG), "task": "detect"}
    target.task = "detect"
    ...
    checkpoint = {
        ...
        "version": ultralytics.__version__, "license": "AGPL-3.0", "cscef_v5_provenance": report,
    }
...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "outputs/cscef_v5/initialization.json")
