"""MinCompat native final-box evaluation and streaming complete archive; never starts training/evaluation implicitly."""
...
import numpy as np
import torch
from mincompat_v2 import ROOT, require, runtime, sha256, topology, write_json, SOURCE_SHA256, VARIANTS, git
from ultralytics.utils import ops
from train_mincompat_v2 import MAIN, paths, run_state
...
def validate_complete(p):
    ...
    require(all((launch / n).is_file() for n in required), "MinCompat direct-start metadata incomplete")
...
def package(variant, destination=None, allow_incomplete=False):
    ...
    destination = Path(destination).resolve() if destination else MAIN / "downloads/mincompat_v2" / variant / f"{variant}_complete_{datetime.now():%Y%m%d_%H%M%S_%f}.tar.gz"
    ...
    logs = ROOT / "outputs/mincompat_v2_command_logs" / variant
    if logs.exists():
    ...
    for name in tracked:
        if name and (name.startswith(("ultralytics-main/ultralytics/", "ultralytics-main/tests/", "docs/mincompat_v2/")) or
                     name in ("ultralytics-main/pyproject.toml", ".gitattributes", "docs/mincompat_v2/c2_args.yaml") or
...
if __name__ == "__main__":
    ...
    if args.mode == "pack-complete":
        ...
        require(args.output is None, "Formal evaluations use independent fixed MinCompat directories")
