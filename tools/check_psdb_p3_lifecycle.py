...
import torch
import train_psdb_p3 as lifecycle
from eval_psdb_p3 import postprocess
    ...
    class FakeModel:
        ...
        def train(self, trainer, **kwargs):
            ...
            t.model = SimpleNamespace(parameters=lambda: iter([parameter]),
                                      named_parameters=lambda: iter([("model.19.psdb.W_o.weight", parameter)]))
...
def check():
    results = {}
    with tempfile.TemporaryDirectory(prefix="psdb-lifecycle-") as temp:
        root = Path(temp)
