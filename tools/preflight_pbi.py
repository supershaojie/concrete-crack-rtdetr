...
from pbi_common import ROOT, VARIANTS, require, sha256, write_json, runtime, verify_model, build_training_model, is_added
from train_pbi import (DEFAULT_MAIN, recipe, code_identity, dataset_identity, verify_initialization,
                       optimizer_coverage, disable_oom_retry, git_head, server_environment)
...
def run(args):
    ...
    try:
        ...
        if not torch.cuda.is_available():
            missing.append("CUDA")
        if report["server_environment"]["status"] != "PASSED":
            missing.append("specified Python3.10 / torch2.1.2+cu121 / RTX4090 target environment")
        ...
        class CapacityTrainer(PBICheckpointTrainer):
            ...
            def optimizer_step(self):
                ...
                require(all(bool(torch.isfinite(p).all()) for p in self.model.parameters()),
                        "Nonfinite parameter after native optimizer/scaler step")
        ...
        def batch_end(trainer):
            ...
            require(current.get("DN", {}).get("dn_num_split", [0])[0] > 0,
                    "Native denoising GT path was not exercised")
        ...
        require(report["capacity"]["effective_updates"] >= args.target_updates,
                "Native GradScaler did not achieve the required effective PBI updates within 16 batches")
        branch = {n: p for n, p in trainer.model.named_parameters() if is_added(n)}
        require(all(torch.isfinite(p).all() for p in branch.values()), "Nonfinite learned PBI parameter")
        require(all(any(step["applied"] and step["gradients"][n]["finite"] and
                ...
                "Not all PBI projections received later finite nonzero gradients")
        ...
        # PBI_PREFLIGHT_DETACH_BEFORE_SAVE_V1
        for handle in handles:
            handle.remove()
        handles.clear()
        with TemporaryDirectory(prefix="diagnostic_", dir=out) as temporary:
            diagnostic = Path(temporary) / "learned_diagnostic.pt"
            # Exercise exactly the formal serializer, with only its output paths
            # redirected to this owned temporary directory. This is not a permit
            # to resume a mid-epoch capacity run as a formal experiment.
            trainer.last = diagnostic
            trainer.best = Path(temporary) / "best.pt"
            trainer.wdir = Path(temporary)
            trainer.save_model()
            report["learned_diagnostic_sha256"] = sha256(diagnostic)
            report["learned_diagnostic_bytes"] = diagnostic.stat().st_size
            from pbi_checkpoint import require_checkpoint_policy
            report["saved_checkpoint_policy"] = require_checkpoint_policy(
                torch.load(diagnostic, map_location="cpu", weights_only=False))
